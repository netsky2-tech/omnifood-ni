...
| 11 | The migration is split into two additive files rather than one, to keep each within the review budget: `1809000000000-CreateHumanAuthorizationCore.ts` (epochs, ack history, ack floor) and `1809010000000-CreateHumanAuthorizationRecoveryAndObservability.ts` (recovery tokens, recovery events, verification events, rollout cohorts). Both are additive; nothing reads the second group until Slice D. | implementer-chosen (deviates from the single file named in `tasks.md` `I-2`) |
| 12 | `down()` drops only objects this migration created and never deletes evidence rows: it removes triggers and functions, then drops the tables that hold no retained evidence in the same file. Evidence tables are dropped only by an explicit operator decision outside the migration path, matching §11. | design-specified ("must not erase evidence") |
