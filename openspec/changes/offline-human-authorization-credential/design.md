...
| 21 | **Dedicated snapshot schema id.** The snapshot carries its own schema id `ohac.staff-policy-snapshot.v1` rather than reusing `ohac.staff-policy-epoch.v1`, because the v1 epoch contract requires a non-empty `targetTerminalId` and `targetPosBuild` and its digest covers the whole body including both, so a terminal-agnostic snapshot can neither parse as `v1` nor reuse the v1 digest; the per-terminal epoch materialized on pull carries its own v1 body and digest (decision 17). | design-specified (terminal-agnostic snapshot), implementer-chosen (schema id) |
| 22 | **Publisher-persisted cohort decision (user decision).** The publisher persists a tenant-level cohort decision with each snapshot: `ELIGIBLE` when an enabled cohort row matches the tenant and the publisher's own `publisherBackendBuild`, otherwise `DISABLED`. The exact POS/backend build-pair check stays a pull-time materialization check, because `targetPosBuild` is only negotiated on the terminal's pull (decision 17) and cannot be known at publication time. | user-decided (tenant-level decision at publication, build pair at pull) |
| 23 | **Replay-based idempotence (user decision).** Decision 16's literal "compares the projected payload digest against the newest snapshot" is unreachable for the snapshot store, because the digest covers its own chain fields (`sequence`, `previousSequence`, `previousDigest`), so a projection chained from the newest snapshot can never reproduce the newest snapshot's digest. The publisher therefore reprojects the freshly loaded records under the newest snapshot's own chain metadata and compares digests — equal digests mean equal canonical bodies, i.e. the source policy is unchanged — and additionally requires the freshly resolved cohort decision to equal the stored one, because the decision is persisted per publication (§4.1 rule 2) and a decision flip is a genuine change whose skip would leave the stored gate state stale. A different build, different entries, or a decision flip republishes with the derived next sequence. | user-decided (replay comparison plus cohort-decision equality) |
...
### 11.5 POS delivery decisions resolved before implementation

The backend half of delivery (§11.4 decisions 24-29) is implemented and covered against a real database. Designing the POS half surfaced five points this design left open or silent, and two of them blocked any implementation, so they are recorded here before any Dart is written.

| # | Decision | Status |
|---|---|---|
| 30 | **Negotiated POS build (user decision).** No value in the POS can serve as the negotiated build today: the only occurrences are a hardcoded `'1.0.0+1'` literal in the controlled-sale runner and the pubspec version, which is not readable at runtime without a package that exposes it. The POS therefore reads its own package version at runtime through `package_info_plus` and sends it verbatim as `posBuild`. Reading the built artifact's own version is the only source that cannot drift from the artifact, and the exact `(pos_build, backend_build)` pair is the cohort gate's key, so a drifted value would silently disable every terminal. When the version cannot be read the POS omits the negotiation parameters entirely and behaves as a legacy client, which is the fail-closed answer: no epoch, no assertion, and no silently wrong build. | user-decided (package version read at runtime) |
| 31 | **Drain gate scope (user decision).** §5.1's gate needs a registry of assertion-bearing outboxes, and the POS has no such outbox yet: eleven pending-work structures exist and none carries an assertion or an epoch sequence, so the gate would have no registrant. The gate, its registration coupler, the blocked `ACK_SUBMITTING` transition, the `OHAC_ACK_DEFERRED_OUTBOX` reason, its bounded retry count, and its quarantine are therefore implemented now as inert structure, exercised by a test registrant, and become load-bearing when DSI-6's credit-note outbox produces the first assertions. Implementing it now is what stops the invariant from being forgotten when the first registrant arrives. | user-decided (implement now, inert until a registrant exists) |
| 32 | **POS slice decomposition.** The POS half lands as separate review units: local epoch, entry, terminal-state, attempt-state, and local-event persistence with its schema migration; then the crash-safe candidate state machine and the acknowledgement client; then the drain gate with its registry and coupler. Persistence comes first because every later unit writes through it. | implementer-chosen |
| 33 | **Legacy verifier projection stays untouched.** The POS keeps writing `UserEntity` and `security_profiles` from `deltas.users`, and login keeps consulting that path, because §16.3 requires a compatibility and deprecation matrix before cohort enablement and nothing here may silently change authentication. The epoch tables are additive, and which path authenticates becomes a decision for the enablement step, not for this one. | design-specified (§16.3), implementer-chosen (scope) |
| 34 | **Attempt-state transaction shape is an evidence question, not a preference.** §6 leaves the choice between a callback closure and a revision compare-and-set open pending proof that the Floor generation in use supports callback transactions. The persistence unit must establish that against the version actually pinned in `pubspec.yaml` and record the answer, rather than picking one and hoping. | design-specified (open point), to be closed by evidence |

One consequence worth stating, because it is easy to lose: the drain gate's deferral reason must be observable to an operator. A terminal that defers its acknowledgement indefinitely while asserting under the previous epoch is a state that needs to be visible and bounded, not one that quietly retries forever.
