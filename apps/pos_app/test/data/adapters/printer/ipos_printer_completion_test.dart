...
void main() {
  ...
  test(
    'prefers text when a production label has both raw and text forms',
    () async {
      final result = await adapter.printProductionBatchLabel(
        ...
        expirationDate: DateTime(2026, 2),
      );
  ...
  test(
    'rejected raw transport returns failure without false success',
    () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(channel, (call) async {
  ...
  test(
    'invalid logo continues with text and a PNG bitmap invokes the bitmap channel',
    () async {
      ...
      final invalid = await adapter.printInvoice(
        invoice,
        items: [item],
      ...
      calls.clear();
      final validDoc = ReceiptDocument(
        businessName: 'Prueba',
        taxRegime: TaxRegime.regimenGeneral,
        ...
        total: 0,
        totalUsd: 0,
        logoRasterBytes: const [137, 80, 78, 71, 13, 10, 26, 10],
      );
  ...
  test(
    'paper width selects logical layout and native per-job render width',
    () async {
      final invoice = Invoice(
        id: 'i',
        number: '1',
      ...
      final narrow = await adapter.printInvoice(
        invoice,
        items: [item],
        payments: const <Payment>[],
        taxRegime: TaxRegime.regimenGeneral,
      ...
      final wide = await adapter.printInvoice(
        invoice,
        items: [item],
        payments: const <Payment>[],
        taxRegime: TaxRegime.regimenGeneral,
      ...
      final narrowAgain = await adapter.printInvoice(
        invoice,
        items: [item],
        payments: const <Payment>[],
        taxRegime: TaxRegime.regimenGeneral,
      ...
      expect(
        narrow.printedText!.split('\n').every((line) => line.length <= 32),
        isTrue,
      );
      expect(
        wide.printedText!.split('\n').every((line) => line.length <= 40),
        isTrue,
      );
      expect(
        narrowAgain.printedText!.split('\n').every((line) => line.length <= 32),
        isTrue,
      );
