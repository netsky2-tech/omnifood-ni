...
void main() {
  ...
  test('returns failure when the print transport is unavailable', () async {
    ...
    test('skips an invalid legacy logo and still prints the receipt text', () async {
    ...
    test('skips a logo larger than the native bitmap safety limit and still prints the receipt text', () async {
    final invoice = Invoice(
      id: 'invoice-1',
    ...
    final result = await adapter.printInvoice(
      ...
      payments: const <Payment>[],
      taxRegime: TaxRegime.regimenGeneral,
      logoRasterBytes: List<int>.filled(IPosPrinterAdapter.maxLogoBytes + 1, 0),
    );
