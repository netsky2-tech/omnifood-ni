...
class _MockCoordinator extends Mock implements DeviceSyncCredentialCoordinator {}
...
void main() {
  ...
  group('DeviceSyncAuthInterceptor onRequest', () {
    test('attaches device access token as Bearer only to /v1/sync/* routes', () async {
      when(() => coordinator.getAccessToken())
    ...
    test('attaches device token to relative path v1/sync/inbound/deltas', () async {
      when(() => coordinator.getAccessToken())
          .thenAnswer((_) async => 'device.access.jwt');
    ...
    test('NEVER attaches device token to admin/onboarding/human endpoints', () async {
      final paths = [
        '/identity/login',
        '/admin/users',
    ...
    test('rejects request when coordinator throws DeviceSyncRevokedException', () async {
      ...
      expect(handler.rejectionError!.error, isA<DeviceSyncRevokedException>());
    });

    test('rejects request when coordinator throws DeviceSyncUnavailableException', () async {
      when(() => coordinator.getAccessToken()).thenThrow(
      ...
      expect(handler.isRejected, isTrue);
      expect(handler.rejectionError, isNotNull);
      expect(handler.rejectionError!.error, isA<DeviceSyncUnavailableException>());
    });
  });

  group('DeviceSyncAuthInterceptor onError (retry handling)', () {
    test('on 401 on /v1/sync/* invalidates token and retries exactly once', () async {
      ...
      final captured = verify(() => clientDio.fetch<dynamic>(captureAny()))
          .captured
          .first as RequestOptions;
      expect(captured.headers['Authorization'], 'Bearer new.refreshed.device.jwt');
      expect(captured.extra['retryAttempt'], 1);
    });

    test('does NOT retry when retryAttempt >= 1 (prevents retry storm)', () async {
      final requestOptions = RequestOptions(
        path: '/v1/sync/batch',
    ...
    test('on 401 on /v1/sync/* when renewal fails with DeviceSyncRevokedException, propagates typed cause via reject', () async {
      when(() => coordinator.invalidateAccessToken()).thenReturn(null);
      when(() => coordinator.getAccessToken()).thenThrow(
        const DeviceSyncRevokedException(reason: 'DEVICE_REVOKED'),
      );
    ...
    test('on 401 on /v1/sync/* when renewal fails with DeviceSyncUnavailableException, propagates typed cause via reject', () async {
      when(() => coordinator.invalidateAccessToken()).thenReturn(null);
      when(() => coordinator.getAccessToken()).thenThrow(
