...
void main() {
  ...
  group('ONB1.8C — Controlled Offline Sale (Production Path & PAID Real)', () {
    ...
    test(
      'fails if attempt is not in RUNNING state (pre-condition gate)',
      () async {
    ...
    test(
      'executes real production checkout, reaches PAID, persists ticket locally, and prints receipt',
      () async {
        ...
        expect(
          ...
          equals('onboarding:activation-sale:$tenantId:$attemptId'),
        );

        // 2. Verify invoice items in SQLite
        final items = await database.invoiceItemDao.getItemsByInvoiceId(
          invoice.id,
        );
        ...
        final payments = await database.paymentDao.getPaymentsByInvoiceId(
          invoice.id,
        );
        ...
        final envelopes = await database.activationOutboxDao
            .getEnvelopesByAttempt(tenantId, attemptId);
        final verificationEnvelope = envelopes.firstWhere(
          (envelope) => envelope.eventType == 'VERIFICATION_SALE',
        );
        final verificationPayload =
            jsonDecode(verificationEnvelope.payloadJson)
        ...
        final receiptCheck = await database.activationCheckResultLocalDao
            .getCheck(tenantId, attemptId, 'SALE_RECEIPT_PATH');
        expect(receiptCheck, isNotNull);
        expect(receiptCheck!.status, equals('PASS'));

        // 6. Verify attempt correlated verificationTicketId
        final updatedAttempt = await database.activationAttemptLocalDao
    ...
    test(
      'retry of attempt does not create a second verification ticket (idempotency)',
      () async {
        await seedPrerequisites();
        ...
        final allInvoices = await database.invoiceDao.getInvoicesBySyncStatus(
          'pending',
        );
    ...
    test(
      'rejects attempt retry when payload parameters conflict (INTEGRITY_CONFLICT)',
      () async {
        await seedPrerequisites();
        ...
        expect(
          conflictingResult.errors,
          anyElement(contains('INTEGRITY_CONFLICT')),
        );
      },
    );
  ...
  group('ONB1.8D — Outbox Durability & Multi-Tenant Isolation', () {
    ...
    Future<void> seedPrerequisites() async {
      ...
      await database.localConfigDao.saveConfig(
        LocalConfigEntity(key: 'dgi_current_seq', value: '10'),
      );
      await database.localConfigDao.saveConfig(
        LocalConfigEntity(key: 'dgi_range_end', value: '1000'),
      );
    ...
    test(
      'consolidates durable outbox envelopes before WAN reconnect',
      () async {
        await seedPrerequisites();
        ...
        final outboxCheck = await database.activationCheckResultLocalDao
            .getCheck(tenantId, attemptId, 'OUTBOX_DURABLE');
    ...
    test(
      'multi-tenant isolation: Tenant A envelopes and sales do not cross Tenant B boundaries',
      () async {
        await seedPrerequisites();
        ...
        final envsA = await database.activationOutboxDao.getEnvelopesByAttempt(
          tenantId,
          attemptId,
        );
        final envsB = await database.activationOutboxDao.getEnvelopesByAttempt(
          tenantB,
          attemptB,
        );
    ...
    test(
      'offline sale and outbox envelopes survive database close and reopen (Disk Persistence)',
      () async {
        final tempDir = await Directory.systemTemp.createTemp(
          'pos_offline_sale_durability_',
        );
        final dbPath = p.join(tempDir.path, 'pos_offline_sale_test.db');

        try {
          final diskDb1 = await $FloorAppDatabase
          ...
          await diskDb1.localConfigDao.saveConfig(
            LocalConfigEntity(key: 'dgi_current_seq', value: '20'),
          );
          await diskDb1.localConfigDao.saveConfig(
            LocalConfigEntity(key: 'dgi_range_end', value: '1000'),
          );
          ...
          final diskMovement = MovementEngineImpl(
            diskInventory,
            mockAlertService,
          );
          ...
          // Reopen diskDb2
          final diskDb2 = await $FloorAppDatabase
              .databaseBuilder(dbPath)
              .build();

          // 1. Recheck Attempt
          final attemptRecovered = await diskDb2.activationAttemptLocalDao
              .getAttemptById(attemptId);
          expect(attemptRecovered, isNotNull);
          expect(
            attemptRecovered!.localStatus,
            equals('LOCAL_ACTIVATION_EVIDENCE_COMPLETE'),
          );
          expect(attemptRecovered.verificationTicketId, equals(ticketId));

          // 2. Recheck Invoice
          final invoiceRecovered = await diskDb2.invoiceDao.getInvoiceById(
            ticketId!,
          );
          ...
          // 3. Recheck Checks
          final checkRecovered = await diskDb2.activationCheckResultLocalDao
              .getCheck(tenantId, attemptId, 'OFFLINE_SALE_PAID');
          expect(checkRecovered, isNotNull);
          expect(checkRecovered!.status, equals('PASS'));

          // 4. Recheck Outbox Envelopes
          final envsRecovered = await diskDb2.activationOutboxDao
              .getEnvelopesByAttempt(tenantId, attemptId);
  ...
  group('TDD Triangulation — Domain Invariants & Edge Cases', () {
    ...
    test(
      'fails if verification product is missing from SQLite catalog',
      () async {
        await database.activationAttemptLocalDao.saveAttempt(
          const ActivationAttemptLocalEntity(
            attemptId: attemptId,
        ...
        expect(
          result.errors,
          anyElement(contains('VERIFICATION_PRODUCT_NOT_FOUND')),
        );
      },
    );
    ...
    test(
      'fails if tenant mismatch occurs between attempt and parameter',
      () async {
        await database.activationAttemptLocalDao.saveAttempt(
          const ActivationAttemptLocalEntity(
            attemptId: attemptId,
    ...
    test(
      'activation_outbox_envelopes enforces uniqueness on (tenant_id, idempotency_key)',
      () async {
        ...
        expect(
          () async => await database.activationOutboxDao.insertEnvelope(
            duplicateEnvelope,
          ),
          throwsA(anything),
        );
    ...
    group(
      'ONB1.8E & ONB1.8F — TTFSS First Successful Sale Claim & Clock Semantics',
      () {
        const tenantId = 'tenant-founder-01';
        ...
        test(
          'creates atomic FirstSuccessfulSaleClaimEntity and emits FIRST_SUCCESSFUL_SALE_OBSERVED outbox envelope',
          () async {
            await seedPrerequisites(
            ...
            final claim = await database.firstSuccessfulSaleClaimDao
                .getClaimByTenantId(tenantId);
            ...
            final envelopes = await database.activationOutboxDao
                .getPendingEnvelopes(tenantId);
            final firstSaleEnv = envelopes.firstWhere(
              (e) => e.eventType == 'FIRST_SUCCESSFUL_SALE_OBSERVED',
            );
            expect(firstSaleEnv, isNotNull);
            expect(
              firstSaleEnv.idempotencyKey,
              equals('onboarding:first-sale:$tenantId'),
            );
            expect(firstSaleEnv.syncStatus, equals('PENDING'));

            final payload =
                jsonDecode(firstSaleEnv.payloadJson) as Map<String, dynamic>;
        ...
        test(
          'write-once invariant: second sale or attempt retry NEVER overwrites winning claim nor emits second FIRST_SUCCESSFUL_SALE_OBSERVED',
          () async {
            await seedPrerequisites(
              serverTimeAnchorAt: '2026-09-04T12:00:00.000Z',
              anchorMonotonicTicks: 0,
            ...
            final claimBefore = await database.firstSuccessfulSaleClaimDao
                .getClaimByTenantId(tenantId);
            ...
            final claimAfter = await database.firstSuccessfulSaleClaimDao
                .getClaimByTenantId(tenantId);
            expect(claimAfter!.ticketId, equals(winningTicketId));
            expect(
              claimAfter.createdAtLocal,
              equals(claimBefore.createdAtLocal),
            );

            // Exactly one FIRST_SUCCESSFUL_SALE_OBSERVED outbox envelope must exist
            final envelopes = await database.activationOutboxDao
                .getPendingEnvelopes(tenantId);
            final firstSaleEnvs = envelopes
                .where((e) => e.eventType == 'FIRST_SUCCESSFUL_SALE_OBSERVED')
                .toList();
        ...
        test(
          'claim survives database restart with real SQLite disk persistence',
          () async {
            final tempDir = await Directory.systemTemp.createTemp(
              'pos_claim_disk_test_',
            );
            final dbFile = File(p.join(tempDir.path, 'pos_claim_test.db'));

            try {
              // Open DB on disk
              var diskDb = await $FloorAppDatabase
                  .databaseBuilder(dbFile.path)
                  .build();

              // Seed configs
              await diskDb.localConfigDao.saveConfig(
                LocalConfigEntity(key: 'dgi_prefix', value: '001-001-01'),
              );
              await diskDb.localConfigDao.saveConfig(
                LocalConfigEntity(key: 'dgi_current_seq', value: '1'),
              ...
              final aRepo = AuthRepositoryImpl(
                diskDb.userDao,
                diskDb.securityProfileDao,
                localAuth,
                mockDio,
                capabilityCache: capCache,
              );
              final audRepo = AuditRepositoryImpl(
                diskDb.auditDao,
                aRepo,
                mockDio,
                'pos-terminal-founder-01',
                capabilityCache: capCache,
                forensicAlertDao: diskDb.forensicAlertDao,
              );
              final invRepo = InventoryRepositoryImpl(
                insumoDao: diskDb.insumoDao,
                recipeDao: diskDb.recipeDao,
                movementDao: diskDb.movementDao,
                movementSyncStateDao: diskDb.movementSyncStateDao,
                supplierDao: diskDb.supplierDao,
                warehouseDao: diskDb.warehouseDao,
                countSessionDao: diskDb.countSessionDao,
                countLineDao: diskDb.countLineDao,
                forensicAlertDao: diskDb.forensicAlertDao,
                uomConversionDao: diskDb.uomConversionDao,
                batchDao: diskDb.batchDao,
                purchaseDao: diskDb.purchaseDao,
                recipeVersionDocumentDao: diskDb.recipeVersionDocumentDao,
                productionOrderDocumentDao: diskDb.productionOrderDocumentDao,
                dio: mockDio,
                database: diskDb,
              );
              ...
              final sRepo = SalesRepositoryImpl(
                database: diskDb,
                invoiceDao: diskDb.invoiceDao,
                itemDao: diskDb.invoiceItemDao,
                paymentDao: diskDb.paymentDao,
                transactionDao: diskDb.salesTransactionDao,
                numberingService: numServ,
                movementEngine: movEng,
              ...
              // Re-open from disk
              diskDb = await $FloorAppDatabase
                  .databaseBuilder(dbFile.path)
                  .build();
              final reloadedClaim = await diskDb.firstSuccessfulSaleClaimDao
                  .getClaimByTenantId(tenantId);
              ...
              final envelopes = await diskDb.activationOutboxDao
                  .getPendingEnvelopes(tenantId);
              expect(
                envelopes.any(
                  (e) => e.eventType == 'FIRST_SUCCESSFUL_SALE_OBSERVED',
