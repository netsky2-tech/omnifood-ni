...
void main() {
  ...
  SaleViewModel createViewModel() {
    final rewardInteraction = LoyaltyRewardInteractionService(
      evaluationService,
    );
    final viewModel = SaleViewModel.withLoyalty(
      salesRepo,
      inventoryRepo,
      authRepo,
      database,
  ...
  group('LV1.4 Real SQLite E2E — Customer Identification (LV1.4A)', () {
    test(
      'identifica cliente por QR, CustomerCode Crockford y teléfono desde SQLite real',
      () async {
        ...
        final qrResult = await identificationService.identify(
          'NHL1:01HXYZ1234567890',
        );
        ...
        final codeResult = await identificationService.identify(
          '01hxyz1234567890',
        );
        ...
        final invalidResult = await identificationService.identify(
          'UNKNOWN_CODE_999',
        );
  ...
  group(
    'LV1.4 Real SQLite E2E — SPEND_POINTS Checkout & Ledger Persistence (LV1.4B–LV1.4D)',
    () {
      test(
        'flujo completo de compra con redención de recompensa y acumulación en ledger SQLite',
        () async {
          final now = DateTime.now().millisecondsSinceEpoch;
          ...
          final identified = await viewModel.identifyCustomer(
            'NHL1:01ABCDEF12345678',
          );
      ...
      test(
        'mutación del carrito re-evalúa y limpia recompensa cuando deja de ser elegible',
        () async {
          final now = DateTime.now().millisecondsSinceEpoch;
      ...
      test(
        'Triangulación — PRODUCT_STAMPS con recompensa FREE_PRODUCT',
        () async {
          final now = DateTime.now().millisecondsSinceEpoch;
          ...
          expect(viewModel.selectedReward, isNotNull);
          expect(
            viewModel.selectedReward!.rewardType,
            equals(RewardType.freeProduct),
          );
      ...
      test(
        'Triangulación — Multi-programa: evaluación independiente de SPEND y VISIT en el mismo ticket',
        () async {
          final now = DateTime.now().millisecondsSinceEpoch;
  ...
  group('LV1.4 Real SQLite E2E — Fiscal Receipt Formatting (LV1.4E)', () {
    test(
      'formatea ticket térmico 58mm y 80mm con bloque Loyalty sin romper formato fiscal DGI',
      () {
        ...
        for (final line in lines58) {
          expect(
            ...
            reason: 'Línea 58mm excede 32 columnas: "$line"',
          );
        }
        ...
        for (final line in lines80) {
          expect(
            ...
            reason: 'Línea 80mm excede 48 columnas: "$line"',
          );
        }
