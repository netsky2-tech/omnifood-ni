...
void main() {
  group('thermal receipt completion regressions', () {
    test(
      'codec handles all Spanish, decomposed, punctuation, emoji, and unsupported-script samples',
      () {
    ...
    test(
      '58mm extreme descriptions, quantities, amounts, modifiers, customer and cashier stay printable',
      () {
        final formatter = ReceiptLayoutFormatter.format58mm();
        ...
        final document = ReceiptDocument(
          ...
          documentNumber: '1',
          date: DateTime(2026),
          cashierName: 'Cajero con nombre extraordinariamente largo',
          customerName: 'Cliente con nombre extraordinariamente largo',
          lines: const [
            ReceiptLine(
              quantity: 1000,
              description:
                  'Supercalifragilisticoespialidoso descripción extremadamente larga',
              unitPrice: amount,
              taxableBase: amount,
              lineSubtotal: amount,
              lineTotal: amount,
    ...
    test(
      '80mm monetary and quantity boundaries align normal totals and preserve fallback values',
      () {
        ...
        for (final amount in amounts) {
          final numericMoney = ReceiptLayoutFormatter.formatMoney(
            amount,
            includeSymbol: false,
          );
          final normal = formatter.formatItemRow(
            quantity: 1,
            name: 'Producto',
            unitPrice: amount,
            total: amount,
          );
          expect(normal.every((line) => line.length <= 40), isTrue);
          expect(normal.join('\n'), contains(numericMoney));
          if (normal.length == 1) {
            expect(
              normal.single.lastIndexOf(numericMoney) + numericMoney.length,
              40,
            );
          }
          for (final quantity in quantities) {
            final rows = formatter.formatItemRow(
              quantity: quantity,
              name: 'Descripción larga con varios modificadores',
              unitPrice: amount,
              total: amount,
            );
        ...
        final fallback = formatter.formatItemRow(
          quantity: 1000000000,
          name: 'Descripción extraordinariamente larga para fallback',
          unitPrice: 999999999999999.99,
          total: 999999999999999.99,
        );
        expect(fallback.every((line) => line.length <= 40), isTrue);
    ...
    test(
      'kitchen, Corte X, Corte Z, and FIFO label retain extreme key information within 32 columns',
      () {
        final kitchen = Receipt58mmFormatter.formatKitchenOrderText(
          ticketId: 'TK-EXTREMO-000000000000',
          orderTitle: 'Orden extraordinariamente larga',
          cashierName: 'Cajero extraordinariamente largo',
          timestamp: DateTime(2026),
          items: const [],
          tableName: 'Mesa extraordinariamente larga',
        );
        final session = CashierSession(
          id: 'id',
          userId: 'user',
          terminalId: 'TERMINAL-EXTRAORDINARIAMENTE-LARGO',
          openedAt: DateTime(2026),
          openingBalanceNio: 9999999.99,
          openingBalanceUsd: 9999999.99,
          expectedNio: 9999999.99,
        );
        final corteX = Receipt58mmFormatter.formatCorteXText(
          session,
          cashierName: 'Cajero extraordinariamente largo',
          totalsByMethod: const {},
        );
        ...
        final label = Receipt58mmFormatter.formatProductionBatchLabelText(
          productName:
              'Producto extraordinariamente largo que no debe truncarse',
          batchCode: 'LOTE-EXTREMO-000000000000000',
          quantity: 999999.99,
          uom: 'kilogramos',
          productionDate: DateTime(2026),
          expirationDate: DateTime(2026, 12, 31),
          operatorName: 'Operador extraordinariamente largo',
        );
        for (final text in [kitchen, corteX, corteZ, label]) {
          _expectWidth(text, 32);
        }
        expect(
          kitchen.replaceAll(RegExp(r'\s+'), ' '),
          contains('Mesa extraordinariamente larga'),
        );
        expect(
          corteX.replaceAll(RegExp(r'\s+'), ''),
          contains('TERMINAL-EXTRAORDINARIAMENTE-LARGO'),
        );
        expect(corteZ, contains('999999'));
        expect(
          label.replaceAll(RegExp(r'\s+'), ' '),
          contains('Producto extraordinariamente largo que no debe truncarse'),
        );
      },
    );

    test(
      'renderer treats receipt document monetary and modifier display fields as authoritative',
      () {
