...
void main() {
  ...
  group(
    'E2E POS Activation: Pre-Offline Checks -> Controlled Offline Sale -> Outbox Durability -> Crash Recovery',
    () {
      ...
      test(
        ...
        () async {
          final tempDir = await Directory.systemTemp.createTemp(
            'pos_activation_e2e_durability_',
          );
          ...
          try {
            ...
            final terminalIdentity = TerminalIdentityService(
              db1.localConfigDao,
            );
            ...
            expect(
              preOfflineSummary.checks.values.every((c) => c.status == 'PASS'),
              isTrue,
            );

            final attemptAfterPre = await db1.activationAttemptLocalDao
            ...
            final salesRepo = SalesRepositoryImpl(
              ...
              processInventoryUseCase: ProcessSaleInventoryUseCase(
                movementEngine,
              ),
              reverseInventoryUseCase: ReverseSaleInventoryUseCase(
                movementEngine,
              ),
            ...
            expect(
              saleResult.attemptStatus,
              equals('LOCAL_ACTIVATION_EVIDENCE_COMPLETE'),
            );
            expect(
              saleResult.checks['OFFLINE_SALE_PAID']!.status,
              equals('PASS'),
            );
            expect(
              saleResult.checks['SALE_RECEIPT_PATH']!.status,
              equals('PASS'),
            );
            expect(saleResult.checks['OUTBOX_DURABLE']!.status, equals('PASS'));
            // One pre-offline hardware test print plus the controlled-sale receipt.
            expect(printerAdapter.printHistory.length, equals(2));
            ...
            expect(
              rehydratedInvoice.idempotencyKey,
              equals('onboarding:activation-sale:$tenantId:$attemptId'),
            );

            // 3. Invoice Items & Payments preserved
            final rehydratedItems = await db2.invoiceItemDao
                .getItemsByInvoiceId(ticketId);
            expect(rehydratedItems.length, equals(1));
            expect(
              rehydratedItems.first.productId,
              equals(verificationProductId),
            );
            expect(rehydratedItems.first.total, equals(65.0));

            final rehydratedPayments = await db2.paymentDao
                .getPaymentsByInvoiceId(ticketId);
            ...
            final allChecks = await db2.activationCheckResultLocalDao
            ...
            expect(
              allChecks.map((c) => c.checkCode).toSet(),
              equals(expectedCheckCodes),
            );

            // 5. Outbox Envelopes are durable, pending, and tamper-proof
            final rehydratedEnvelopes = await db2.activationOutboxDao
            ...
            for (final env in rehydratedEnvelopes) {
              ...
              final computedHash = sha256
                  .convert(utf8.encode(env.payloadJson))
                  .toString();
            ...
            // -------------------------------------------------------------
            final reloadedNumbering = DgiNumberingServiceImpl(
              db2.localConfigDao,
            );
            ...
            final retryResult = await reloadedRunner
                .executeControlledOfflineSale(
                  const ControlledSaleParams(
                    tenantId: tenantId,
                    attemptId: attemptId,
                    cashierUserId: authorizedUserId,
            ...
            final finalInvoices = await db2.invoiceDao.getInvoicesBySyncStatus(
              'pending',
            );
