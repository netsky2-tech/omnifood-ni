...
deleted_rows="$(psql_admin -d "${SCRATCH_DB}" -tAc \
  ...
  | tr -d ' ')"
expected_deleted_rows=41
# Derivation: the list above holds exactly 41 migration names (the pre-slice-10
# 39 plus slice 10's ConvertKardexAndConfigForAllPolicies1809340000000 and
# slice 11's AddTenantSlug1809350000000), and
# the run below proves the count against the real ledger (40 -> 41 would fail
