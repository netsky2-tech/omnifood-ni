...
const materialize = (input: unknown): Buffer => {
  ...
  if (recipe.repeat) {
    ...
    const count = integer(repeated.count);
    const byte = integer(repeated.byte);
    if (count > MAX_RECIPE_BYTES || byte > 255)
  ...
  if (bytes.length > MAX_RECIPE_BYTES)
    throw new Error('materialization exceeds bounds');
  return bytes;
};
...
const containsNumber = (value: AuditV3Value): boolean =>
  value.kind === 'array'
    ? value.values.some(containsNumber)
    : value.kind === 'object'
      ? value.entries.some((entry) => containsNumber(entry.value))
      : false;

describe('scanNumberFreeJson', () => {
  it.each(load('rejections.jsonl'))(
    'rejects $id at its exact phase and byte',
    (row) => {
      expect(scanNumberFreeJson(materialize(row.raw))).toEqual({
        ok: false,
        error: { code: row.code, offset: row.offset },
      });
    },
  );

  it.each(load('canonical-valid.jsonl'))(
    'accepts $id with a number-free AST',
    (row) => {
  ...
  it('preserves decoded object entry order and supplementary keys', () => {
    ...
    expect(
      ordered.ok &&
        ordered.value.kind === 'object' &&
        ordered.value.entries.map(({ key }) => key),
    ).toEqual(['b', 'a']);
    const scalar = scanNumberFreeJson(Buffer.from('{"\\ue400":"b","𐀀":"a"}'));
    expect(
      scalar.ok &&
        scalar.value.kind === 'object' &&
        scalar.value.entries.map(({ key }) => key),
    ).toEqual(['\ue400', '𐀀']);
  });

  it('applies phase precedence and never returns a partial AST', () => {
    expect(scanNumberFreeJson(Buffer.from('{"a":1,"\\u0061":2}'))).toEqual({
      ok: false,
      error: { code: 'AUDIT_V3_DUPLICATE_KEY', offset: 7 },
    });
    expect(scanNumberFreeJson(Buffer.from('[1,2]'))).toEqual({
      ok: false,
      error: { code: 'AUDIT_V3_NUMBER_FORBIDDEN', offset: 1 },
    });
  });
  ...
  it('rejects a large numeric array without spreading numeric offsets', () => {
    ...
    expect(scanNumberFreeJson(raw)).toEqual({
      ok: false,
      error: { code: 'AUDIT_V3_NUMBER_FORBIDDEN', offset: 1 },
    });
  });
