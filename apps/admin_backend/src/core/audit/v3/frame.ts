...
export function buildAuditV3Frame(
  fields: AuditV3FrameFields,
  canonicalMetadata: Buffer,
): Result<Buffer> {
  try {
    const parts: Buffer[] = [Buffer.from('OFA3', 'ascii')];
    const required = [
      fields.user_id,
      fields.resolved_action,
      fields.device_id,
      fields.timestamp,
      fields.sequence_no,
      fields.prev_hash,
    ...
    required.forEach((value, index) => {
      if (typeof value !== 'string' || !scalar(value))
        fail(CODE.FRAME_INVALID, index);
      if ((index === 1 || index === 3) && value.length === 0)
        fail(CODE.FRAME_INVALID, index);
      if (
        index === 4 &&
        (!/^(?:0|[1-9][0-9]*)$/.test(value) || BigInt(value) > 4_294_967_295n)
      )
        fail(CODE.FRAME_INVALID, index);
      add(0x01, Buffer.from(value, 'utf8'), index);
    });
    [fields.metodo_autorizacion, fields.usuario_autorizador_id].forEach(
      (field, offset) => {
        const index = offset + 6;
        if (!field || typeof field !== 'object')
          fail(CODE.FRAME_INVALID, index);
        if (field.state === FIELD_STATE.ABSENT)
          add(0x00, Buffer.alloc(0), index);
        else if (field.state === FIELD_STATE.NULL)
          add(0x03, Buffer.alloc(0), index);
        else if (
          field.state === FIELD_STATE.TEXT &&
          typeof field.value === 'string' &&
          scalar(field.value)
        )
          add(0x01, Buffer.from(field.value, 'utf8'), index);
