...
const expectProof = (bytes: Buffer, proofValue: unknown): void => {
  ...
  if (typeof proof.exact_hex === 'string')
    expect(bytes.toString('hex')).toBe(proof.exact_hex);
  else {
...
describe('canonicalizeNumberFreeJson', () => {
  it.each(load('canonical-valid.jsonl'))(
    'emits canonical authority $id exactly',
    (row) => {
  ...
  it.each(load('rejections.jsonl'))(
    'preserves scanner rejection $id',
    (row) => {
      expect(canonicalizeNumberFreeJson(materialize(row.raw))).toEqual({
        ok: false,
  ...
  it('sorts keys by unsigned UTF-16 units with shorter prefixes first', () => {
    expect(success('{"":"2","𐀀":"1","aa":"4","a":"3"}').toString()).toBe(
      '{"a":"3","aa":"4","𐀀":"1","":"2"}',
    );
  });
  ...
  it('returns no partial output on failure', () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: { code: 'AUDIT_V3_NUMBER_FORBIDDEN', offset: 6 },
    });
