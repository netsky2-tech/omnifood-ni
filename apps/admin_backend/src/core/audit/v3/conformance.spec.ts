...
const injectedReceipt = resolve(
  tmpdir(),
  `audit-v3-conformance-${process.pid}.json`,
);
...
describe('audit v3 cross-runtime conformance', () => {
  ...
  it('hashes equivalent LF and CRLF implementation source identically', () => {
    expect(hashSource(Buffer.from('const a = 1;\nconst b = 2;\n'))).toBe(
      hashSource(Buffer.from('const a = 1;\r\nconst b = 2;\r\n')),
    );
  });

  (process.env.AUDIT_V3_REAL_RUNTIME === '1' ? it : it.skip)(
    'matches the real Dart runtime',
    () => {
  ...
  it('proves all authority groups and writes a deterministic receipt', () => {
    ...
    expect(Object.keys(first.implementations)).toContain(
  ...
  it('fails closed with a row diagnostic when runtime output is tampered', () => {
    expect(() =>
      runConformance(ROOT, receipt, (_root, expected) => {
  ...
  it.each([
    [
      '63 rows',
      (_root: string, expected: readonly unknown[]) =>
        JSON.stringify(expected.slice(1)),
    ],
    [
      '65 rows',
      (_root: string, expected: readonly unknown[]) =>
        JSON.stringify([...expected, expected[0]]),
    ],
    ...
    expect(() => runConformance(ROOT, receipt, runner)).toThrow(
      'dart runtime must return exactly 64 rows',
    );
  });
  ...
  it.each([
    ...
    'rejects %s successful Dart stdout with bounded stderr',
    (_label, stdout) => {
      const stderr = `useful context ${'x'.repeat(600)} hidden tail`;
      expect(() =>
        runConformance(ROOT, injectedReceipt, () => ({
          status: 0,
          stdout,
          stderr,
        })),
      ).toThrow(
        /^dart runtime produced invalid JSON: .*stderr: useful context x{10}/,
      );
      expect(() =>
        runConformance(ROOT, injectedReceipt, () => ({
          status: 0,
  ...
  it('preserves the original Dart launch diagnostic', () => {
    const runner: RuntimeRunner = () => ({
      ...
      stdout: null,
      stderr: null,
      error: new Error('spawn dart ENOENT'),
    });
    expect(() => runConformance(ROOT, receipt, runner)).toThrow(
      'dart runtime launch failed: spawn dart ENOENT',
    );
  });

  it.each([
    [
      'launch failed: spawn dart ENOENT',
      { status: null, stdout: null, error: new Error('spawn dart ENOENT') },
    ],
    ...
    const stderr = `useful context ${'x'.repeat(600)} hidden tail`;
    const invoke = () =>
      runConformance(ROOT, injectedReceipt, () => ({ ...result, stderr }));
  ...
  it('reports a deterministic Dart timeout diagnostic', () => {
    const error = Object.assign(new Error('spawnSync dart ETIMEDOUT'), {
      code: 'ETIMEDOUT',
    });
    const stderr = `useful timeout context ${'x'.repeat(600)} hidden tail`;
    const invoke = () =>
      runConformance(ROOT, receipt, () => ({
        status: null,
        stdout: null,
        stderr,
        error,
    ...
    expect(invoke).toThrow(
      /^dart runtime timed out after 90000ms; stderr: useful timeout context x{10}/,
    );
    expect(invoke).not.toThrow('hidden tail');
  });
});
