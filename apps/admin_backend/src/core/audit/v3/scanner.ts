import {
  AUDIT_V3_ERROR_CODE as CODE,
  type AuditV3ErrorCode,
  type AuditV3ObjectEntry,
  type AuditV3Value,
  type Result,
} from './types';
...
const fail = (code: AuditV3ErrorCode, offset: number): never => {
  throw new Failure(code, offset);
};
...
export function scanNumberFreeJson(rawUtf8: Buffer): Result<AuditV3Value> {
  try {
    ...
    const offset = (index: number): number =>
      index >= source.length ? rawUtf8.length : byteOffsets[index];
    let position = 0;
    const unicode: number[] = [];
    const duplicates: number[] = [];
    let numberOffset: number | undefined;
    const limits: number[] = [];
    ...
    const string = (): { value: string; start: number } => {
      ...
      while (position < source.length) {
        ...
        if (hex.length < 4 || bad >= 0)
          fail(
            CODE.INVALID_JSON,
            offset(position + (bad < 0 ? hex.length : bad)),
          );
        ...
        if (first >= 0xd800 && first <= 0xdbff) {
          ...
          if (
            source.slice(position, position + 2) === '\\u' &&
    ...
    const value = (depth: number): AuditV3Value => {
      ...
      if (character === '[') {
        position++;
        const node: Extract<AuditV3Value, { kind: 'array' }> = {
          kind: 'array',
          values: [],
        };
        stack.push({ node, depth, count: 0, afterValue: false });
        return node;
      }
      if (character === '{') {
        position++;
        const node: Extract<AuditV3Value, { kind: 'object' }> = {
          kind: 'object',
          entries: [],
        };
        stack.push({
      ...
      fail(
        CODE.INVALID_JSON,
        position >= source.length ? rawUtf8.length : start,
      );
    };
    whitespace();
    const root = value(0);
    while (stack.length) {
      ...
      if (frame.afterValue) {
        ...
        if (source[position++] !== ',')
          fail(CODE.INVALID_JSON, offset(position - 1));
        frame.afterValue = false;
        whitespace();
      } else if (frame.count === 0 && source[position] === close) {
        position++;
        stack.pop();
        continue;
      }
      const crossing = offset(position);
      frame.count++;
      if (frame.count > 10_000) limits.push(crossing);
      if (frame.node.kind === 'array')
        (frame.node.values as AuditV3Value[]).push(value(frame.depth + 1));
      else {
        if (source[position] !== '"')
          fail(
            CODE.INVALID_JSON,
            position >= source.length ? rawUtf8.length : offset(position),
          );
        const key = string();
        if (frame.seen.has(key.value)) duplicates.push(key.start);
        frame.seen.add(key.value);
        whitespace();
        if (source[position++] !== ':')
          fail(CODE.INVALID_JSON, offset(position - 1));
        const entry: AuditV3ObjectEntry = {
          key: key.value,
          value: value(frame.depth + 1),
        };
    ...
    whitespace();
    if (position !== source.length) fail(CODE.INVALID_JSON, offset(position));
    const candidate = (code: AuditV3ErrorCode, values: number[]): void => {
      if (values.length) fail(code, Math.min(...values));
    };
    candidate(CODE.INVALID_UNICODE, unicode);
    candidate(CODE.DUPLICATE_KEY, duplicates);
    if (numberOffset !== undefined) fail(CODE.NUMBER_FORBIDDEN, numberOffset);
    candidate(CODE.LIMIT_EXCEEDED, limits);
    return { ok: true, value: root };
  } catch (error: unknown) {
    if (error instanceof Failure)
      return { ok: false, error: { code: error.code, offset: error.offset } };
    throw error;
  }
}
