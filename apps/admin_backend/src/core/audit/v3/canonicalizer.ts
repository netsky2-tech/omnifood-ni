...
class Writer {
  ...
  write(text: string): void {
    for (let start = 0; start < text.length && !this.exceeded; ) {
      ...
      if (this.length + chunk.length > MAX) {
        this.exceeded = true;
        return;
      }
...
function writeEscaped(writer: Writer, value: string): void {
  ...
  for (let index = 0; index < value.length; index++) {
    ...
    const replacement =
      escapes[character] ??
      (code < 0x20 ? `\\u00${code.toString(16).padStart(2, '0')}` : undefined);
    if (!replacement) continue;
    writer.write(value.slice(run, index));
    writer.write(replacement);
    run = index + 1;
  }
  writer.write(value.slice(run));
  writer.write('"');
}
...
export function canonicalizeNumberFreeJson(rawUtf8: Buffer): Result<Buffer> {
  ...
  const stack: Action[] = [scanned.value];
  while (stack.length && !writer.exceeded) {
    ...
    if (action.kind === 'escaped') {
      writeEscaped(writer, action.value);
      continue;
    }
    if (action.kind === 'null') {
      writer.write('null');
    ...
    if (action.kind === 'boolean') {
      writer.write(action.value ? 'true' : 'false');
      continue;
    }
    ...
    if (action.kind === 'array')
      action.values.forEach((value, index) =>
        values.push(index ? text(',') : text(''), value),
      );
    else
      [...action.entries]
        .sort((a, b) => compareUtf16(a.key, b.key))
        .forEach((entry, index) =>
          values.push(
            index ? text(',') : text(''),
            escaped(entry.key),
  ...
  if (writer.exceeded)
    return {
      ok: false,
      error: { code: AUDIT_V3_ERROR_CODE.LIMIT_EXCEEDED, offset: MAX },
    };
