...
import { EnforceRecipeCatalogRls1809260000000 } from './1809260000000-EnforceRecipeCatalogRls';
 ...
 * all five target tables. The predicate FORMS themselves are not re-spelled
 * here: they are imported from the policy helper, the single source of that
 * vocabulary.
 */
describe('EnforceRecipeCatalogRls1809260000000', () => {
  const migration = new EnforceRecipeCatalogRls1809260000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on all five recipe-catalog tables', async () => {
    ...
    it('forces row level security on all five recipe-catalog tables', async () => {
      const { sql } = await collectSql('up');
  ...
  describe('tenant isolation policies', () => {
    ...
    it('resolves each table independently: a table still on varchar keeps the text form while the uuid tables get the uuid form', async () => {
      const { sql } = await collectSql('up', {
        // Triangulation for the per-table resolution: pretend `batches`
        // alone still sits at the varchar end of the rebind while the other
        // four are uuid.
        batches: 'character varying',
      });

      // batches on varchar: 5 text halves (select + insert + update's USING
      // and WITH CHECK + delete); the other four tables on uuid: 20 uuid
      // halves — same shape, independent resolution.
