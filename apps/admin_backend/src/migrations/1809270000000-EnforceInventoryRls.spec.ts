...
import { EnforceInventoryRls1809270000000 } from './1809270000000-EnforceInventoryRls';
 ...
 * entry default to `uuid`, the type migration 1759000000002 leaves behind on
 * all four target tables. The predicate FORMS themselves are not re-spelled
 * here: they are imported from the policy helper, the single source of that
 * vocabulary.
 */
describe('EnforceInventoryRls1809270000000', () => {
  const migration = new EnforceInventoryRls1809270000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on all four inventory tables', async () => {
    ...
    it('forces row level security on all four inventory tables', async () => {
      const { sql } = await collectSql('up');
    ...
    it('touches exactly the four target tables and no others', async () => {
      const { sql } = await collectSql('up');
  ...
  describe('tenant isolation policies', () => {
    it('creates one policy per command for every table using the project naming convention (16 policies total)', async () => {
      const { sql } = await collectSql('up');
      ...
      // 4 tables x 4 commands = exactly 16 policies, no extras.
    ...
    it('resolves the predicate exactly once per table, one independent resolution per table', async () => {
      ...
      // The resolver must be consulted once for EACH of the four tables, in
    ...
    it('carries the resolved uuid-form predicate in every USING/WITH CHECK half (all four columns are uuid)', async () => {
      const { sql } = await collectSql('up');

      // 4 tables x (select 1 + insert 1 + update 2 + delete 1) = 20 halves,
    ...
    it('resolves each table independently: a table still on varchar keeps the text form while the uuid tables get the uuid form', async () => {
      const { sql } = await collectSql('up', {
        // Triangulation for the per-table resolution: pretend `warehouses`
        // alone still sits at the varchar end of the rebind while the other
        // three are uuid.
        warehouses: 'character varying',
      });

      // warehouses on varchar: 5 text halves (select + insert + update's
      // USING and WITH CHECK + delete); the other three tables on uuid:
      // 15 uuid halves — same shape, independent resolution.
      expect(sql.split(PREVIOUS_TENANT_RLS_PREDICATE).length - 1).toBe(5);
      expect(sql.split(TENANT_RLS_PREDICATE).length - 1).toBe(15);
    });
  ...
  describe('down migration safety', () => {
    it('drops exactly the policies it created, for all four tables', async () => {
