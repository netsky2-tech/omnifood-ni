...
import { EnforceCatalogRls1809250000000 } from './1809250000000-EnforceCatalogRls';
 ...
 * both target tables. The predicate FORMS themselves are not re-spelled
 * here: they are imported from the policy helper, the single source of that
 * vocabulary.
 */
describe('EnforceCatalogRls1809250000000', () => {
  const migration = new EnforceCatalogRls1809250000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on both catalog tables', async () => {
    ...
    it('forces row level security on both catalog tables', async () => {
      const { sql } = await collectSql('up');
  ...
  describe('tenant isolation policies', () => {
    ...
    it('resolves each table independently: a table still on varchar keeps the text form while the uuid table gets the uuid form', async () => {
      const { sql } = await collectSql('up', {
        insumos: 'character varying',
      });

      // insumos on varchar: 5 text halves (select + insert + update's USING
      // and WITH CHECK + delete); products on uuid: 5 uuid halves — same
      // shape, independent resolution.
