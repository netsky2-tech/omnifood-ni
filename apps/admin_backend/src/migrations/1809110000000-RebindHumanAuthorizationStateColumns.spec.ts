...
import { RebindHumanAuthorizationStateColumns1809110000000 } from './1809110000000-RebindHumanAuthorizationStateColumns';

/**
 * The authored catalog truth for the human authorization domain, slice part
 * 2: four tables, 10 policies, every one of them policy-carrying. This is
 ...
 *
 * The five tables rebound by unit B2.2 are deliberately absent: they belong
 * to the previous unit, and no statement in this migration may touch them.
...
// these four columns is varchar(128) today, but each is measured, not assumed.
const PREVIOUS_TYPES: Record<string, string> = {
  human_auth_verification_events: 'character varying(128)',
  human_auth_rollout_cohorts: 'character varying(128)',
  human_auth_tenant_publication_state: 'character varying(128)',
  human_auth_policy_snapshots: 'character varying(128)',
};

describe('RebindHumanAuthorizationStateColumns1809110000000', () => {
  const migration = new RebindHumanAuthorizationStateColumns1809110000000();
  ...
  it('reports the migration name required by the migrations ledger', () => {
    expect(migration.name).toBe(
      'RebindHumanAuthorizationStateColumns1809110000000',
    );
  });

  it('carries exactly 10 authored policy rows matching the catalog convention', () => {
    // The catalog on the scratch database holds exactly these 10 rows for the
    // four tables 180902/180904/180905 created policies on; the spec pins the
    // count so a target silently added or dropped here cannot pass.
    expect(POLICY_ROWS).toHaveLength(10);

    // Policy counts per table: append-only tables carry select+insert; the
    // mutable ones add update. All four tables in this unit are
    // policy-carrying.
    const expectedCommands: Record<(typeof REBOUND_TABLES)[number], string[]> =
      {
        human_auth_verification_events: ['insert', 'select'],
        human_auth_rollout_cohorts: ['insert', 'select', 'update'],
        human_auth_tenant_publication_state: ['insert', 'select', 'update'],
        human_auth_policy_snapshots: ['insert', 'select'],
      };
  ...
  it('emits no view statement and does not touch the B2.2 tables', async () => {
    const { queryRunner, queries } = createQueryRunner();
    ...
    // The five remaining human authorization tables belong to B2.2, which is
    // already merged; `sys_parametros_config` was rebound by slice B1 too.
    for (const table of [
