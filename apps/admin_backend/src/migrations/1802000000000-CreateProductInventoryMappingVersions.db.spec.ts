...
describe('Migration 1802000000000-CreateProductInventoryMappingVersions DB spec', () => {
  ...
  itDb('enforces tenant composite FKs, RLS isolation, history immutability and down guard', async () => {
    await withIsolatedSchema('mapping_migration_test', async ({ queryRunner, schema, tenantRole }) => {
      const migration = new CreateProductInventoryMappingVersions1802000000000();
      ...
      await queryRunner.query(`GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA "${schema}" TO "${tenantRole}"`);
      await queryRunner.query(`SET ROLE "${tenantRole}"`);

      await queryRunner.query(`SELECT set_config('app.tenant_id', $1, false)`, [tenantB]);
      const rowsB = await queryRunner.query(`SELECT * FROM product_inventory_mapping_versions`);
      expect(rowsB).toHaveLength(0); // Tenant B sees nothing

      await queryRunner.query(`SELECT set_config('app.tenant_id', $1, false)`, [tenantA]);
      const rowsA = await queryRunner.query(`SELECT * FROM product_inventory_mapping_versions`);
      expect(rowsA).toHaveLength(1); // Tenant A sees its mapping

      // 4. Immutability guard tests
      // A. DELETE rejected
      await expect(
        queryRunner.query(`DELETE FROM product_inventory_mapping_versions WHERE id = $1`, [mappingA.id]),
      ...
      await expect(migration.down(queryRunner)).rejects.toThrow('refusing to remove product mapping history');
    });
  });
});
