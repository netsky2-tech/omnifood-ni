...
import { EnforceCustomerLoyaltyRls1809280000000 } from './1809280000000-EnforceCustomerLoyaltyRls';
 ...
 * entry default to `uuid`, the type every target column has in a
 * migrations-built schema today: the bootstrap loyalty tables were created
 * uuid (1759000000004) and the two legacy varchar columns were rebound to
 * uuid by 1809140000000. The predicate FORMS themselves are not re-spelled
 * here: they are imported from the policy helper, the single source of that
 * vocabulary.
 */
describe('EnforceCustomerLoyaltyRls1809280000000', () => {
  const migration = new EnforceCustomerLoyaltyRls1809280000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on all five customer/loyalty tables', async () => {
    ...
    it('forces row level security on all five customer/loyalty tables', async () => {
      const { sql } = await collectSql('up');
    ...
    it('touches exactly the five target tables and no others', async () => {
      const { sql } = await collectSql('up');

      const alters = sql.match(/ALTER TABLE "[a-z_]+"/g) ?? [];
      expect([...new Set(alters)].sort()).toEqual([
        'ALTER TABLE "customer_loyalty_account_projection"',
  ...
  describe('tenant isolation policies', () => {
    it('creates one policy per command for every table using the project naming convention (20 policies total)', async () => {
      const { sql } = await collectSql('up');
    ...
    it('carries the resolved uuid-form predicate in every USING/WITH CHECK half (all five columns are uuid)', async () => {
      const { sql } = await collectSql('up');

      // 5 tables x (select 1 + insert 1 + update 2 + delete 1) = 25 halves,
      // all carrying the single-source uuid predicate.
      expect(sql.split(TENANT_RLS_PREDICATE).length - 1).toBe(25);
      expect(sql).not.toContain(PREVIOUS_TENANT_RLS_PREDICATE);
      expect(sql).not.toContain('tenant_id::text');
    });

    it('resolves each table independently: a table still on varchar keeps the text form while the uuid tables get the uuid form', async () => {
      const { sql } = await collectSql('up', {
        // Triangulation for the per-table resolution: pretend `customers`
        // alone still sits at the varchar end of the rebind (it was created
        // character varying by 1789000000000 and rebound by 1809140000000)
        // while the other four are uuid.
        customers: 'character varying',
      });

      // customers on varchar: 5 text halves (select + insert + update's
      // USING and WITH CHECK + delete); the other four tables on uuid:
      // 20 uuid halves — same shape, independent resolution.
      expect(sql.split(PREVIOUS_TENANT_RLS_PREDICATE).length - 1).toBe(5);
      expect(sql.split(TENANT_RLS_PREDICATE).length - 1).toBe(20);
    });
