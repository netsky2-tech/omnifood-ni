...
import { EnforceOnboardingTemplateRls1809230000000 } from './1809230000000-EnforceOnboardingTemplateRls';
 ...
 * entry default to `uuid`, the type migrations 1809080000000 + 1809140000000
 ...
 */
describe('EnforceOnboardingTemplateRls1809230000000', () => {
  const migration = new EnforceOnboardingTemplateRls1809230000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on all three template/provenance tables', async () => {
    ...
    it('forces row level security on all three template/provenance tables', async () => {
      const { sql } = await collectSql('up');
    ...
    it('touches exactly the three target tables and no others', async () => {
      ...
      expect([...new Set(alters)].sort()).toEqual([
        'ALTER TABLE "legacy_onboarding_migration_receipts"',
        'ALTER TABLE "onboarding_template_applications"',
  ...
  describe('tenant isolation policies', () => {
    ...
    it('resolves each table independently: a table still on varchar keeps the text form while the uuid tables get the uuid form', async () => {
      const { sql } = await collectSql('up', {
        onboarding_template_seed_links: 'character varying',
      });

      // seed_links on varchar: 5 text halves (select + insert + update's
      // USING and WITH CHECK + delete); the other two tables on uuid:
      // 10 uuid halves — same shape, independent resolution.
