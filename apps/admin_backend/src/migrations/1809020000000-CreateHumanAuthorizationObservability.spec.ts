...
describe('CreateHumanAuthorizationObservability1809020000000', () => {
  ...
  it('resolves the tenant predicate per table and emits the uuid form when the stubbed type is uuid', async () => {
    ...
    expect(sql).toContain(
      `CREATE POLICY human_auth_verification_events_tenant_select ON human_auth_verification_events FOR SELECT USING (${uuidPredicate});`,
    );
    expect(sql).toContain(
      `CREATE POLICY human_auth_verification_events_tenant_insert ON human_auth_verification_events FOR INSERT WITH CHECK (${uuidPredicate});`,
    );
    expect(sql).toContain(
      `CREATE POLICY human_auth_rollout_cohorts_tenant_select ON human_auth_rollout_cohorts FOR SELECT USING (${uuidPredicate});`,
    );
    expect(sql).toContain(
      `CREATE POLICY human_auth_rollout_cohorts_tenant_insert ON human_auth_rollout_cohorts FOR INSERT WITH CHECK (${uuidPredicate});`,
    );
    expect(sql).toContain(
      `CREATE POLICY human_auth_rollout_cohorts_tenant_update ON human_auth_rollout_cohorts FOR UPDATE USING (${uuidPredicate}) WITH CHECK (${uuidPredicate});`,
    );
  ...
  it('emits the text-cast predicate when tenant_id is still varchar, never the uuid cast', async () => {
    ...
    expect(sql).toContain(
      `CREATE POLICY human_auth_verification_events_tenant_select ON human_auth_verification_events FOR SELECT USING (${textPredicate});`,
    );
    expect(sql).toContain(
      `CREATE POLICY human_auth_rollout_cohorts_tenant_select ON human_auth_rollout_cohorts FOR SELECT USING (${textPredicate});`,
    );
    expect(sql).not.toContain('::uuid');
  });

  it('refuses to emit tenant policies when the table has no tenant_id column', async () => {
    // The first table resolved is human_auth_verification_events, so the error names it.
    await expect(collectSql('up', null)).rejects.toThrow(
      /'human_auth_verification_events' has no tenant_id column/,
    );
  });
