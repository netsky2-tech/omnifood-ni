...
import { EnforceOnboardingImportRls1809240000000 } from './1809240000000-EnforceOnboardingImportRls';
 ...
 * entry default to `uuid`, the type migrations 1809070000000 + 1809140000000
 * leave behind on all three target tables. The predicate FORMS themselves are
 * not re-spelled here: they are imported from the policy helper, the single
 * source of that vocabulary.
 */
describe('EnforceOnboardingImportRls1809240000000', () => {
  const migration = new EnforceOnboardingImportRls1809240000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on all three import/integrity tables', async () => {
    ...
    it('forces row level security on all three import/integrity tables', async () => {
      const { sql } = await collectSql('up');
    ...
    it('touches exactly the three target tables and no others', async () => {
      const { sql } = await collectSql('up');

      const alters = sql.match(/ALTER TABLE "[a-z_]+"/g) ?? [];
      expect([...new Set(alters)].sort()).toEqual([
        'ALTER TABLE "legacy_import_integrity_reports"',
        'ALTER TABLE "product_import_sessions"',
        'ALTER TABLE "staging_importacion_productos"',
      ]);
    });
  });
  ...
  describe('tenant isolation policies', () => {
    ...
    it('resolves the predicate exactly once per table, one independent resolution per table', async () => {
      ...
      // The resolver must be consulted once for EACH of the three tables, in
    ...
    it('carries the resolved uuid-form predicate in every USING/WITH CHECK half (all three columns are uuid post-rebind)', async () => {
      const { sql } = await collectSql('up');

      // 3 tables x (select 1 + insert 1 + update 2 + delete 1) = 15 halves,
    ...
    it('resolves each table independently: a table still on varchar keeps the text form while the uuid tables get the uuid form', async () => {
      const { sql } = await collectSql('up', {
        product_import_sessions: 'character varying',
      });

      // product_import_sessions on varchar: 5 text halves (select + insert +
      // update's USING and WITH CHECK + delete); the other two tables on
      // uuid: 10 uuid halves — same shape, independent resolution.
  ...
  describe('down migration safety', () => {
    it('drops exactly the policies it created, for all three tables', async () => {
