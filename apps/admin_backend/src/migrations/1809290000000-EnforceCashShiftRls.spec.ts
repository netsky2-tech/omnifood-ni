...
import { EnforceCashShiftRls1809290000000 } from './1809290000000-EnforceCashShiftRls';
 ...
 * both target tables (cash_movements:477, cash_shift_sessions:507 — uuid
 * NOT NULL from birth, so this migration is pure policy work). The predicate
 * FORMS themselves are not re-spelled here: they are imported from the
 * policy helper, the single source of that vocabulary.
 */
describe('EnforceCashShiftRls1809290000000', () => {
  const migration = new EnforceCashShiftRls1809290000000();
  ...
  describe('row level security enforcement', () => {
    it('enables row level security on both cash tables', async () => {
    ...
    it('forces row level security on both cash tables', async () => {
      const { sql } = await collectSql('up');
  ...
  describe('tenant isolation policies', () => {
    it('creates one policy per command for every table using the project naming convention (8 policies total)', async () => {
      const { sql } = await collectSql('up');
    ...
    it('carries the resolved uuid-form predicate in every USING/WITH CHECK half (both columns are uuid)', async () => {
      const { sql } = await collectSql('up');
    ...
    it('resolves each table independently: a table still on varchar keeps the text form while the uuid tables get the uuid form', async () => {
      const { sql } = await collectSql('up', {
        // Triangulation for the per-table resolution: pretend
        // `cash_movements` alone still sits at the varchar end of the rebind
        // while cash_shift_sessions is uuid.
        cash_movements: 'character varying',
      });

      // cash_movements on varchar: 5 text halves (select + insert + update's
      // USING and WITH CHECK + delete); cash_shift_sessions on uuid:
      // 5 uuid halves — same shape, independent resolution.
