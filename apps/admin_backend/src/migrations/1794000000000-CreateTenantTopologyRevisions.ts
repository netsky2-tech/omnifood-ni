...
export class CreateTenantTopologyRevisions1794000000000 implements MigrationInterface {
  ...
  async up(runner: QueryRunner): Promise<void> {
    await runner.query(
      `CREATE TABLE IF NOT EXISTS tenant_topology_revisions (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id varchar NOT NULL, contract_version integer NOT NULL, revision integer NOT NULL, topology jsonb NOT NULL, hash varchar NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), CONSTRAINT uq_tenant_topology_revisions_revision UNIQUE (tenant_id, revision))`,
    );
    await runner.query(
      `CREATE OR REPLACE FUNCTION reject_tenant_topology_revision_mutation() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'tenant_topology_revisions is immutable'; END; $$ LANGUAGE plpgsql`,
    );
    await runner.query(
