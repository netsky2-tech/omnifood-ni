...
export class AddTenantCapabilityEvent1785000000000 implements MigrationInterface {
  ...
  async up(runner: QueryRunner): Promise<void> {
    await runner.query(
      `CREATE TABLE IF NOT EXISTS tenant_capability_event (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id varchar NOT NULL, actor_user_id varchar NOT NULL, previous_version varchar NOT NULL, new_version varchar NOT NULL, contract_version integer NOT NULL, reason varchar NOT NULL, revision integer NOT NULL, created_at timestamptz NOT NULL DEFAULT now(), CONSTRAINT uq_tenant_capability_event_revision UNIQUE (tenant_id, revision))`,
    );
    await runner.query(
    ...
    await runner.query(
      `CREATE OR REPLACE FUNCTION reject_tenant_capability_event_mutation() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'tenant_capability_event is append-only'; END; $$ LANGUAGE plpgsql`,
    );
    await runner.query(
      'DROP TRIGGER IF EXISTS tenant_capability_event_immutable ON tenant_capability_event',
    );
    await runner.query(
