...
/**
 * Forces tenant row level security on the recipe-catalog tables `recipes`,
 * `recipe_versions`, `recipe_details`, `uom_conversions` and `batches`
 * (issue #512 T3 slice 2 part B).
 *
 ...
 *   varchar/text column keeps the text comparison. All five columns are uuid
 ...
 *   append-only-in-practice tables. `recipe_versions` has no production
 *   DELETE path today — prior versions are deactivated rather than deleted,
 *   and replacing a published or effective version is rejected in the
 *   service — so its tenant-local DELETE policy is inert today but kept
 *   for the uniform per-command set. `recipe_details` DOES have a
 *   production DELETE path (`replaceExistingVersion` in recipe.service.ts,
 *   reached from the POS ingest path, deletes the outgoing detail rows
 *   before rewriting them), so its DELETE policy is required, not
 *   incidental. The uniform per-command policy set is the declared
 *   contract (T3's final ratchet slice narrows declared sets explicitly,
 *   never by omission here).
 * - No service binding belongs to this migration: part A of slice 2 already
 *   bound every production path touching these tables to the tenant-bound
 *   transaction manager, which is what makes these policies observable
 *   instead of silently breaking pooled reads.
 */
const TABLES = [
  'recipes',
  'recipe_versions',
...
export class EnforceRecipeCatalogRls1809260000000 implements MigrationInterface {
  name = 'EnforceRecipeCatalogRls1809260000000';
