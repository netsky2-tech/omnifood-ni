...
export class LoyaltyService {
  ...
  async findAllPrograms(
    ...
    return runInTenantTransaction(
      ...
      async (manager) => {
        ...
        return manager.getRepository(LoyaltyProgram).find({
          where,
  ...
  async findOneProgram(tenantId: string, id: string): Promise<LoyaltyProgram> {
    return runInTenantTransaction(this.dataSource, tenantId, (manager) =>
      this.findProgram(manager, tenantId, id),
    );
  }
  ...
  async updateProgram(
    ...
    return runInTenantTransaction(
      ...
      tenantId,
      async (manager) => {
        const program = await this.findProgram(manager, tenantId, id);
        ...
        return manager.getRepository(LoyaltyProgram).save(program);
      },
    );
  }
  ...
  async findRewardsByProgram(
    ...
    return runInTenantTransaction(
      ...
      tenantId,
      async (manager) => {
        await this.findProgram(manager, tenantId, programId);
        return manager.getRepository(RewardDefinition).find({
  ...
  async findOneReward(
    ...
    return runInTenantTransaction(this.dataSource, tenantId, (manager) =>
      this.findReward(manager, tenantId, rewardId),
    );
  }
  ...
  async createReward(
    ...
    return runInTenantTransaction(
      ...
      async (manager) => {
        ...
        const rewardRepo = manager.getRepository(RewardDefinition);
        ...
        const saved = await rewardRepo.save(reward);
  ...
  async updateReward(
    ...
    return runInTenantTransaction(
      ...
      tenantId,
      async (manager) => {
        const reward = await this.findReward(manager, tenantId, rewardId);
        ...
        // Bump program config version (same unit: loyalty_programs + loyalty_rewards)
        ...
        program.config_version = program.config_version + 1;
        await manager.getRepository(LoyaltyProgram).save(program);
  ...
  async activateReward(
    ...
    return runInTenantTransaction(
      ...
      async (manager) => {
        ...
        return manager.getRepository(RewardDefinition).save(reward);
      },
    );
  }
  ...
  async getCustomerBalance(
    ...
    return runInTenantTransaction(
      ...
      tenantId,
      async (manager) => {
        const projection = await manager
        ...
        if (projection && projection.projection_version > 0) {
          return {
            balanceUnits: projection.balance_units,
            source: 'projection',
          };
        }
  ...
  async getCustomerLoyaltyAccounts(
    ...
    return runInTenantTransaction(this.dataSource, tenantId, (manager) =>
      manager.getRepository(CustomerLoyaltyAccountProjection).find({
  ...
  /**
   * Manager-taking read helpers shared by the transactional units (issue #512
   * slice 4): public methods must never call each other's transactional
   * wrappers, so every unit reads through these helpers instead of the public
   * `findOneProgram`/`findOneReward` units.
   */
