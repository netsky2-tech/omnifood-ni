...
describe('OnboardingFeatureRolloutService (ONB1.10E — TDD)', () => {
  ...
  it('allows enabling individual flags and isolates between tenants', () => {
    ...
    expect(service.isEnabled(tenantA, OnboardingFeatureFlag.SESSION_V1)).toBe(true);
    expect(service.isEnabled(tenantB, OnboardingFeatureFlag.SESSION_V1)).toBe(false);
  });
  ...
  it('validates cutover order dependencies strictly (Rule ONB1.10E)', () => {
    ...
    expect(
      result.violations.some((v) => v.includes('requires onboarding.session_v1')),
    ).toBe(true);
    expect(
  ...
  it('advances through 10 cutover stages in correct sequence without skipping dependencies', () => {
    ...
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.SESSION_V1)).toBe(true);
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.SETUP_CENTER_V1)).toBe(false);

    // Stage 2: M2 state-based Setup Center authority
    service.applyCutoverStage(tenantId, 2);
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.SETUP_CENTER_V1)).toBe(true);

    // Stage 3: M3 template safe writer
    service.applyCutoverStage(tenantId, 3);
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.TEMPLATE_SAFE_V1)).toBe(true);

    // Stage 4: M4 product import safe writer
    service.applyCutoverStage(tenantId, 4);
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.IMPORT_CONTRACT_V1)).toBe(true);

    // Stage 6: M5 required config
    service.applyCutoverStage(tenantId, 6);
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.REQUIRED_CONFIG_V1)).toBe(true);

    // Stage 7/8: Activation cloud & runner
    service.applyCutoverStage(tenantId, 8);
    expect(service.isEnabled(tenantId, OnboardingFeatureFlag.ACTIVATION_V1)).toBe(true);
  ...
  it('triangulation: trims tenant IDs and handles edge whitespace seamlessly', () => {
    service.setFlag('  tenant-padded  ', OnboardingFeatureFlag.SESSION_V1, true);
    expect(service.isEnabled('tenant-padded', OnboardingFeatureFlag.SESSION_V1)).toBe(true);
    expect(service.isEnabled('  tenant-padded  ', OnboardingFeatureFlag.SESSION_V1)).toBe(true);
  });
  ...
  it('triangulation: returns false safely when tenantId is empty or null', () => {
    ...
    expect(service.isEnabled(null as any, OnboardingFeatureFlag.SESSION_V1)).toBe(false);
    expect(service.isEnabled(undefined as any, OnboardingFeatureFlag.SESSION_V1)).toBe(false);
  });
