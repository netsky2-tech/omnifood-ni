...
describe('OnboardingSession & EnsureOnboardingStarted (Real PostgreSQL DB)', () => {
  ...
  it('creates single primary session and enforces write-once onboardingStartedAt', async () => {
    await withSessionIsolatedSchema('onb_session_core', async ({ service, tenantAId }) => {
      // 1. Initial start
      ...
      expect(first.lifecycleState).toBe(OnboardingLifecycleState.SETUP_IN_PROGRESS);
      ...
      expect(new Date(second.onboardingStartedAt!).getTime()).toBe(recordedStartedAt);
      expect(second.optimisticVersion).toBe(2);
      expect(new Date(second.lastActivityAt!).getTime()).toBeGreaterThanOrEqual(recordedStartedAt);
    });
  });

  it('guarantees tenant isolation: Tenant A and Tenant B have separate unpolluted sessions', async () => {
    await withSessionIsolatedSchema('onb_session_iso', async ({ service, tenantAId, tenantBId }) => {
  ...
  it('tolerates concurrent start calls without duplicating sessions or violating constraints', async () => {
    await withSessionIsolatedSchema('onb_session_race', async ({ service, tenantAId }) => {
      ...
      const fulfilled = results.filter(
        (r): r is PromiseFulfilledResult<OnboardingSession> => r.status === 'fulfilled',
      );
  ...
  it('persists and enforces unique constraint on (tenant_id, idempotency_key)', async () => {
    await withSessionIsolatedSchema('onb_idem_record', async ({ dataSource, tenantAId }) => {
