...
async function withAcquisitionIsolatedSchema(
  ...
  try {
    ...
    // governing configuration version. This harness scopes everything with
    // the DataSource `schema` option (no search_path is set), so the view
    // name and its table reference are schema-qualified.
    await dataSource.query(`
      CREATE OR REPLACE VIEW "${schema}".v_sys_parametros_config_active
      ...
      SELECT DISTINCT ON (tenant_id, param_key)
        ...
        created_by,
        created_at
      FROM "${schema}".sys_parametros_config
...
describe('ONB1.5 — Fiscal + Catalog Acquisition UX & SALE_READY Transition (PostgreSQL Real DB)', () => {
  it('enforces permission guards on fiscal and catalog writes: Cashier and default Manager are rejected with 403', async () => {
    await withAcquisitionIsolatedSchema(
      'onb15_perms',
      async ({ app, cashierToken, managerToken }) => {
  ...
  it('completes Fiscal Setup and reaches SALE_READY through Manual Minimal Product (AC-04, AC-06, AC-07, AC-08, AC-27)', async () => {
    await withAcquisitionIsolatedSchema(
      'onb15_manual',
      async ({ app, ownerToken, dataSource, tenantId }) => {
        ...
        expect(sessionRes.body.session.lifecycleState).toBe(
          OnboardingLifecycleState.SETUP_IN_PROGRESS,
        );
        expect(sessionRes.body.readiness.saleReady).toBe(false);
        expect(sessionRes.body.readiness.blockers).toContain(
        ...
        expect(readinessAfterFiscal.body.fiscal.minimumConfigurationValid).toBe(
          true,
        );
        expect(readinessAfterFiscal.body.blockers).toEqual([
          'CATALOG_NO_SELLABLE_PRODUCTS',
        ]);
        ...
        expect(manualProdRes.body.session.lifecycleState).toBe(
        ...
        expect(summaryRes.body.sampleProducts[0].costStatus).toBe(
          'COST_PENDING',
        );
