    ...
    await withIdempotencyIsolatedSchema(
      ...
      async ({ coordinator, tenantId }) => {
        ...
        if (lease1.state === 'ACQUIRED') {
          expect(lease1.record.status).toBe(
        ...
        if (leaseReplay.state === 'ALREADY_COMPLETED') {
          expect(leaseReplay.result).toEqual({
            applied: true,
            productsCreated: 2,
          });
        }
  ...
  it('enforces optimistic session concurrency in PostgreSQL, preventing lost updates', async () => {
    await withIdempotencyIsolatedSchema(
      ...
      async ({ sessionService, tenantId }) => {
        ...
        expect(updatedB.lifecycleState).toBe(
