  ...
  try {
    ...
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: 'SystemParametersConfigRepository', useValue: dataSource.getRepository(SystemParametersConfig) },
        { provide: 'ProductRepository', useValue: dataSource.getRepository(Product) },
        { provide: 'InsumoRepository', useValue: dataSource.getRepository(Insumo) },
        { provide: 'RecipeRepository', useValue: dataSource.getRepository(Recipe) },
        { provide: 'RecipeVersionRepository', useValue: dataSource.getRepository(RecipeVersion) },
        { provide: 'RecipeDetailRepository', useValue: dataSource.getRepository(RecipeDetail) },
        { provide: 'UomConversionRepository', useValue: dataSource.getRepository(UomConversion) },
        { provide: 'WarehouseRepository', useValue: dataSource.getRepository(Warehouse) },
        { provide: 'SupplierRepository', useValue: dataSource.getRepository(Supplier) },
        { provide: 'InventoryMovementRepository', useValue: dataSource.getRepository(InventoryMovement) },
        { provide: 'IndustryTemplateRepository', useValue: dataSource.getRepository(IndustryTemplate) },
        { provide: 'TemplateInsumoRepository', useValue: dataSource.getRepository(TemplateInsumo) },
        { provide: 'TemplateProductRepository', useValue: dataSource.getRepository(TemplateProduct) },
        { provide: 'TemplateRecipeItemRepository', useValue: dataSource.getRepository(TemplateRecipeItem) },
        { provide: 'TemplateApplicationRepository', useValue: dataSource.getRepository(TemplateApplication) },
        { provide: 'TemplateSeedLinkRepository', useValue: dataSource.getRepository(TemplateSeedLink) },
        { provide: 'ImportStagingRepository', useValue: dataSource.getRepository(ImportStaging) },
        { provide: 'ProductImportSessionRepository', useValue: dataSource.getRepository(ProductImportSession) },
        { provide: 'LegacyImportIntegrityReportRepository', useValue: dataSource.getRepository(LegacyImportIntegrityReport) },
        { provide: 'LegacyOnboardingMigrationReceiptRepository', useValue: dataSource.getRepository(LegacyOnboardingMigrationReceipt) },
        { provide: 'OnboardingSessionRepository', useValue: dataSource.getRepository(OnboardingSession) },
        { provide: 'OnboardingIdempotencyRecordRepository', useValue: dataSource.getRepository(OnboardingIdempotencyRecord) },
        { provide: 'FiscalConfigRevisionRepository', useValue: dataSource.getRepository(FiscalConfigRevision) },
        { provide: 'ActivationAttemptRepository', useValue: dataSource.getRepository(ActivationAttempt) },
        { provide: 'ActivationCheckResultRepository', useValue: dataSource.getRepository(ActivationCheckResult) },
        { provide: 'ActivationFollowUpRepository', useValue: dataSource.getRepository(ActivationFollowUp) },
        { provide: 'OnboardingTelemetryEventRepository', useValue: dataSource.getRepository(OnboardingTelemetryEvent) },
        { provide: 'ChangeLogRepository', useValue: dataSource.getRepository(ChangeLog) },
        ...
        { provide: IDENTITY_READINESS_PORT, useClass: IdentityReadinessAdapter },
        ...
        { provide: INVENTORY_READINESS_PORT, useClass: InventoryReadinessAdapter },
        { provide: COSTING_READINESS_PORT, useClass: CostingReadinessAdapter },
        { provide: OPERATIONS_READINESS_PORT, useClass: OperationsReadinessAdapter },
      ],
    }).compile();
    ...
    const rolloutService = moduleFixture.get<OnboardingFeatureRolloutService>(OnboardingFeatureRolloutService);
    const activationService = moduleFixture.get<ActivationService>(ActivationService);
    const telemetryService = moduleFixture.get<OnboardingTelemetryService>(OnboardingTelemetryService);
    const customerSaleObserver = moduleFixture.get<OnboardingCustomerSaleObserver>(OnboardingCustomerSaleObserver);
...
describe('ONB1.10D: Full Regression of the 74 Normative Architecture Scenarios (PostgreSQL Real DB)', () => {
  it('covers Scenarios 1–8: Session, Lifecycle, Monotonicity and Audit Reconciler (22.1)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec1', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
      ...
      expect(session1.lifecycleState).toBe(OnboardingLifecycleState.SETUP_IN_PROGRESS);
      ...
      expect(startRes2.body.session.onboardingStartedAt).toBe(originalStartedAt);
      ...
      expect(recRes.body.session.lifecycleState).toBe(OnboardingLifecycleState.SALE_READY);
      ...
      expect(recDegradeRes.body.session.lifecycleState).toBe(OnboardingLifecycleState.SETUP_IN_PROGRESS);
      expect(recDegradeRes.body.session.saleReadyFirstAt).toBe(firstSaleReadyAt);
      ...
      await sessionRepo.update(
        { tenantId: tenantAId },
        { lifecycleState: OnboardingLifecycleState.ACTIVATED, activatedAt: new Date() },
      );
      const recActivated = await request(app.getHttpServer())
        .get('/onboarding/session')
        .set('Authorization', `Bearer ${ownerTokenA}`);
      expect(recActivated.body.session.lifecycleState).toBe(OnboardingLifecycleState.ACTIVATED);
    });
  });

  it('covers Scenarios 9–14: Idempotency Leases, Stale Takeovers and Conflicts (22.2)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec2', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
      ...
      if (lease1.state === 'ACQUIRED') {
        await coordinator.completeSuccess(lease1.record.id, { success: true });
      }
  ...
  it('covers Scenarios 15–24: Safe Industry Templates, Provenance and Recipes (22.3)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec3', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
      ...
      const prodCount = await dataSource.getRepository(Product).count({ where: { tenant_id: tenantAId } });
      ...
      const appliedSeeds = await dataSource.getRepository(TemplateSeedLink).find({
      ...
      expect(recipes[0].suggestion_state).toBe(RecipeSuggestionState.SUGGESTED);
  ...
  it('covers Scenarios 25–35: Canonical CSV Import, Barcode Isolation and Modes (22.4)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec4', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
      ...
      const stagingRows = await dataSource.getRepository(ImportStaging).find({
        where: { tenant_id: tenantAId, token_sesion_importacion: sessionToken },
      });
  ...
  it('covers Scenarios 36–39: Fiscal Fingerprint, Revision & Inbound POS Config (22.5)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec5', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
      ...
      const currentRev = await revRepo.findOne({ where: { tenant_id: tenantAId } });
  ...
  it('covers Scenarios 40–55: Activation Authority, Checks, Checks Severity & Cleanups (22.6)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec6', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
  ...
  it('covers Scenarios 56–60: TTFSS First-Sale Claim, Monotonic Clock & Confidence (22.7)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec7', async ({ app, dataSource, tenantAId, customerSaleObserver }) => {
      // Initialize session
      await dataSource.getRepository(OnboardingSession).save({
        tenantId: tenantAId,
        lifecycleState: OnboardingLifecycleState.SALE_READY,
      ...
      const sessionAfterResend = await dataSource.getRepository(OnboardingSession).findOne({
        where: { tenantId: tenantAId },
      });
      expect(sessionAfterResend!.firstCustomerSaleAt).toEqual(session!.firstCustomerSaleAt);
    });
  });

  it('covers Scenarios 61–68: Multi-Tenant Boundary, RBAC and Zero-Secrets Telemetry (22.8)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec8', async ({
      app,
      dataSource,
      tenantAId,
      tenantBId,
      ...
      const events = await dataSource.getRepository(OnboardingTelemetryEvent).find({
        where: { tenantId: tenantAId },
      });
  ...
  it('covers Scenarios 69–74: Migration Reconciliation, No Fake TTFSS & W9 Preserved (22.9)', async () => {
    await with74ScenariosIsolatedSchema('norm74_sec9', async ({ app, dataSource, tenantAId, ownerTokenA }) => {
      ...
      const reportRepo = dataSource.getRepository(LegacyImportIntegrityReport);
      ...
      const remediateRes = await request(app.getHttpServer())
        ...
        .send({
          ...
          justification: 'Physical inventory count signed off by warehouse manager',
        });
      ...
      const reconcileRes = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${ownerTokenA}`)
        .send({
          justification: 'Closing legacy baseline without fabricating sales timestamp',
        });
      expect(reconcileRes.status).toBe(201);
      expect(reconcileRes.body.decision).toBe(LegacyMigrationDecision.LEGACY_BASELINE_CLOSED);
      expect(reconcileRes.body.evidence_json.measurementEligible).toBe(false);
      expect(reconcileRes.body.evidence_json.firstSuccessfulSaleAt).toBeNull();

      // 74. W9 baseline tests confirmados
      const receipts = await dataSource.getRepository(LegacyOnboardingMigrationReceipt).find({
        where: { tenant_id: tenantAId },
      });
