...
### D-21 (2026-09-25) — the authorization-letter ruling, verbatim from the owner

> Respecto al código de autorización: no existe rango para sistemas computarizados, son consecutivos; los rangos se usan para facturas manuales o papel preimpreso donde se autoriza una orden de trabajo con rango específico. El formato del código suele ser DGI-SFC-2024-00123, RES-SFC-145-2025 — no podemos usar una máscara: sería limitar el sistema a un formato que desconocemos. Lo más sano es un campo string de 50 caracteres con placeholder solo como ayuda; validar guiones y barras. Es importante tener fecha de emisión y fecha de vencimiento, para informar al owner que su código está próximo a vencer (aviso 15 o 30 días antes). Para los consecutivos: `consecutivo_inicial` (ya lo tenemos) y `consecutivo_actual` auto-incremental en otro campo. Series: sucursal única con 1 caja → consecutivo puramente numérico (la DGI solo exige numeración consecutiva, progresiva y sin huecos); sucursal única con 2+ cajas → obligatorio diferenciar secuencias (Caja 1: A-001…, Caja 2: B-001…).

This resolves the #554 blocker in an unexpected direction: the letter was awaited to supply *numbers*, but the ruling supplies a *model* — and the model retires the range. **D-8 is unblocked**: there is nothing to renew/amplify automatically because there is no range to amplify; the expiry warning (fixed 30 days) replaces the hypothetical SLA logic D-8 froze.
...
| **D-21** | **(2026-09-25) There is no "rango" for computarizados — only consecutive numbering.** Rangos exist only for manual/pre-printed invoices (orden de trabajo with a specific range). The computerized model is: authorization code (string ≤50, format unknown — e.g. DGI-SFC-2024-00123, RES-SFC-145-2025 — no mask, validate only hyphens/slashes charset, placeholder as help), emission + expiry dates (warn the owner 30 days before expiry), `consecutivo_inicial` (starting number) and `consecutivo_actual` (auto-incremental cursor). Series: single branch + 1 caja → purely numeric consecutive, serie irrelevant; 2+ cajas in one premises → differentiated per-terminal sequences (A-001…/B-001…); multi-sucursal → per-branch series. DGI requires only consecutiva, progresiva, sin huecos. | Supersedes the *range* framing of #554: B2a's nullable `sequenceEnd` was directionally right, but the range concept itself is dead — the exhaustion gate and range UI die with it. Scope: full #554 redesign (backend fields + POS form/numbering + expiry warning); per-terminal series deferred to its own issue (design note only). Owner decisions relayed with the ruling: warning lead time **fixed 30 days**; multi-caja **design-prepared, not implemented**. |
