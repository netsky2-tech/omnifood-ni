...
## Round 2 answers received (2026-09-24) — directives D-12 to D-19

The accountant answered P8–P11 plus three items we had not put to her. Recorded near-verbatim because these are compliance text, and because **one of them reverses a design decision shipped an hour earlier**.

| # | Directive | What it changes here |
|---|---|---|
| **D-12** | Same-day is the invoice's **local calendar date**, not the shift. A shift **may** cross midnight. Worked example given: a ticket issued 23/09 21:40 must **not** accept a same-day void on 24/09 00:20; one issued 24/09 00:15 must. `CashShift` and `localIssueDate` are to be **kept separate**, and **Reporte X/Z must not define this rule.** | Re-read D-11's "durante su turno actual" as a conjunction, not the whole test. B1a-4's `ShiftMembership` is necessary but no longer sufficient — see the correction below. |
| **D-13** | Reprint must reproduce an **immutable fiscal snapshot taken at issuance** — header, authorization, number, date, lines and totals. A reprint **never reads current fiscal data**. Mark `REIMPRESIÓN`, record reprint date/time and audit trail, and **consume no new correlativo**. | #547's open question is answered as **option 1**: this *is* a schema change. B1r grows a snapshot column/table and cannot be built on the live-config print path. |
| **D-14** | **Prohibit VOID of a previous date outright.** Cross-day correction is an administrative document / Nota de Crédito. **SOHO is the emitter, not NHILOS.** For V1 that lives in **Backoffice** under permission `ISSUE_CREDIT_NOTE`; until it exists, **block the flow in the POS** and use a validated administrative procedure. Never show success if the backend did not accept the document. | The POS's existing "REALIZAR DEVOLUCIÓN" button must go dark in V1, not merely get honest wording. `6dcadf13` fixed the lie; D-14 removes the action. New unit B1c. |
| **D-15** | SOHO V1 self-void = own invoice **+** open current shift **+** same local calendar date, mandatory reason, full audit trail. **No amount or count threshold, no mandatory supervisor.** Record void metrics so the policy can be evaluated later. | The guard is a **three-predicate conjunction** (`VOID_OWN_CURRENT_SHIFT_SALE`), and the reason it must be a permission stands. V5's void register stops being optional polish: it is the instrument she asked for to decide this later. |
| **D-16** | Delete `1–1000` as an authority or default. **`sequenceEnd` must be nullable until real documentation exists.** Ask SOHO for the computerized-invoicing authorization letter and the actual configuration/series. | Confirms D-1 and hardens it: nullable is a stronger requirement than "no hardcoded default". Any `end: 1000` / `10000` / `1000000` literal is now a defect, including the three-way inconsistency found in #540. |
| **D-17** | `fiscalAuthorizationNumber` is **P0** and must print in the **bottom-right of the invoice**. Also store the backing date/document. | B1d is no longer a wiring nicety inside Batch 2; it is a **go-live blocker on the printed document**, and it is the fix for the orphan field in #540 S-1. |
| **D-18** | Dead terminal: there is **no** filing deadline/procedure to encode. | B2b's SLA logic stays unbuilt. D-8's refusal to auto-renew stands, now with a reason instead of a hunch. |

**Correction to B1a-4, found by reading D-12 against D-15.** D-11's sentence was a conjunction and I implemented one of its three legs as if it were the whole rule. `shiftId` is still required (predicate 2), but predicate 3 is the invoice's own local calendar date and must not be derived from the shift — the explicit warning that a shift may cross midnight is exactly the case where the two disagree. Two consequences worth stating plainly:

- **`localIssueDate` must be captured at issuance, not computed at void time.** `createdAt` is epoch millis; deriving a local date from it re-interprets the ticket under whatever timezone the device reports later. Her example is a midnight-crossing scenario, which is precisely where a timezone change moves a ticket across the boundary that decides whether it is voidable. Same reasoning as D-13's snapshot: the fiscal fact is fixed when the document is issued.
- **The two axes have different unknown-ness.** A pre-migration row has `shiftId == null` (unknown), but its local date is computable from `createdAt` for every row that has ever existed. So the date predicate is never `unknown`, and `ShiftMembership`'s tri-state must not be stretched to cover it — a guard that returns "unknown" for a fact it can compute would strand every old ticket.

**Priority consequence.** D-17 makes the authorization number **P0 on the printed document**, and D-16 makes the sequence range a documentation dependency rather than a default. Those two, not the void UI, are now the shortest path to "this can legally emit a ticket". New order: **B1a-4 date leg correction → B1d authorization number (P0) → B1a-2 void → B1c hide credit note → B1r reprint snapshot**, with B2a blocked on SOHO's letter per D-16.
...
**D-20 — partial returns policy (2026-09-24, closing the question raised after B1c-1).** No partial returns in the POS, ever, for the pilot: before charging, the cashier freely edits items/quantities; after issuance, a same-day error at the counter is handled by **FULL VOID + REISSUE** — there is no PARTIAL_VOID. Later partial corrections go through the Backoffice credit-note flow when it ships; until then, the administrative/manual procedure for historical corrections applies. **Design the credit-note domain to support partial lines/quantities in the future — the backend already validates per-item refund limits — but build no partial interaction on the tablet.** And the separation principle: **if the original payment was card, the fiscal void/credit and the payment refund are separate events — never assume one executed the other.** Rationale: don't turn a café edge case into a mini-ERP before opening day; partial credit notes stay in the architecture for NHILOS's general-product future (retail will demand them). The arqueo/cash-model fix (#529 comment) is attacked after this batch.

**Rounds 2 sent as `docs/operations/preguntas-contadora-round-2.md` (2026-09-24); answers recorded as D-12→D-18; follow-up units tracked as issues: cloud projection gap #551, unscoped `getActiveSession()` #552, B1c #553, B2a #554.** Numbering continues #535's Q1–Q7, so the mapping is: **P8** = this fiscal-date / midnight-crossing question (blocks B1a-2's edge case and the X/Z reports), **P9** = the reprint header question from #547 (blocks B1r), **P10** = who emits a cross-day credit note and from where — raised because the backend still rejects `CREDIT_NOTE` over device transport, so the accountant's own "flujo administrativo separado" currently cannot run from the tablet, which is a day-1 procedure gap and not a code gap — **P11** = whether supervisor approval is wanted later as internal policy, explicitly labelled non-DGI. Plus three still-missing facts already asked in #535: the authorized folio range, the authorization resolution number/date, and the dead-terminal filing deadline. The document states plainly that `VOID_OWN_CURRENT_SHIFT_SALE` **does not exist yet** rather than letting her read a design decision as shipped behaviour.
