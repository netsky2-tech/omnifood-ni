...
- Correction (2026-09-20): the bullet above overstates the caller inventory. `ActivationReconnectSyncRunner` — the component that carries the attempt-finalization path (`apps/pos_app/lib/data/services/activation_reconnect_sync_runner.dart:234` → `apps/pos_app/lib/data/adapters/activation/dio_activation_sync_port.dart:100,113`) — is constructed in five test/integration files, not only the attached Q80 harness: `apps/pos_app/test/data/services/activation_reconnect_sync_runner_test.dart:115,349`, `apps/pos_app/test/integration/onb1_10_founder_pilot_rehearsal_e2e_test.dart:384,507`, `apps/pos_app/test/integration/activation_lifecycle_m6_closure_e2e_test.dart:386`, `apps/pos_app/test/integration/activation_fault_injection_e2e_test.dart:297`, and `apps/pos_app/integration_test/onb1_10_founder_pilot_q80_e2e_test.dart:467`. The accurate statement is that no production-wired path creates or finalizes the attempt (already softened in FREEZE-04C).
...
- This supersedes the 2026-09-19 cohort, which recorded two fields as not captured and invalidated two runs. The previous numbers are kept in the evidence document as history rather than deleted.
- Capturing the two fields cost one instrument change (PR #435): both live on the first-sale claim the reconnect phase already read, so no new query was needed — the confidence verbatim and a SHA-256 of the outbox event id.
- The release identity was minted a third time for this cohort, and all three identities are recorded with the reason each changed, because each corresponds to the instrument that actually ran.

Earlier cohort (2026-09-19, release `4b13e4e`): 5/5 runs passed, TTFSS 973/937/827/927/1006 ms, but `clockConfidence` and `firstSaleClaimEventIdHash` were not captured and two runs had to be invalidated and re-executed after the view-grant defect (#431) left their sessions carrying an older onboarding start.
