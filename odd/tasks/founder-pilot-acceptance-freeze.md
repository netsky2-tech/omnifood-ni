...
FREEZE-07 — the 5/5 TTFSS cohort. Each run needs its own freshly provisioned target through the corrected path, because the seed is not idempotent and `setup` is single-use per tenant. Each run also needs the device kept awake for its whole duration, a single adb server holding the device, and operator observation of the physical ticket. `omnifood_founder_pilot_official_third_rehearsal` is pristine and ready for the first cohort run; four more targets must be provisioned.
