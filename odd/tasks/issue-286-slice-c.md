...
10. **`pg_policies` deparses the column side with parentheses, so the obvious LIKE pattern silently passes.** The stored form is `((tenant_id)::text = current_setting('app.tenant_id'::text, true))`, not `tenant_id::text = ...`. A check written as `LIKE '%tenant_id::text%'` therefore matches nothing and reports zero offenders whether the defect is present or not, which is worse than no check because it manufactures confidence. Use `%(tenant_id)::text%` to detect the index-losing form. And do not reach for a bare `%::text%` either: every policy in the target form contains `::text` legitimately, because the predicate casts `current_setting('app.tenant_id'::text, true)` to uuid.
...
| **C.4** | `c4854cc` (#400) | 6 tables rebound, 4 policies recreated (5 tables column-only); ratchet **7 → 1** on a base that already contains C.2 and C.3. Harness exit 0 in both scenarios: `1 / 0 / 0 / 0` verbatim, `uuid tenant_id tables: 58 → 64`, `entity uuid mismatches: 0`, `ledger rows removed: 27 / 27`. Spec 5/5; `1768000000000-CreateCatalogValues.spec.ts` and `1795000000000-AddLoyaltyV1Columns…spec.ts` unmodified. Suites green on the new base, which now also carries C.3's spec: `npm test` 2186 tests, `test:db` 219, `test:e2e` 394. The three `DB_PASSWORD`-gated specs activated and green (3 / 7). No fixture was edited, which is the empirical refutation of the predicted fixture work. |
| **C.5** | `3c4caa0` (#401) | 1 table rebound, 4 policies recreated; ratchet **1 → 0**, the end of the slice. Harness exit 0 in both scenarios: `0 / 0 / 0 / 0` verbatim, `uuid tenant_id tables: 64 → 65`, `entity uuid mismatches: 0`, `ledger rows removed: 27 / 27`. The four `credit_note_invoice_items_*` policies verified present exactly once in the scratch catalog, all four in the setting-cast form with no column-side `::text`, and `invoice_items` still `FORCE ROW LEVEL SECURITY`. That rests on scenario 2 re-running `1782000000000`, whose type-aware resolver re-emits the uuid predicate because the column is uuid by then — independently confirmed by the emitted SQL, not only by the counter. Spec 5/5; `1782000000000-AddCreditNoteProvenance.spec.ts` and `1809060000000-AlignInvoiceTenantPolicyPredicate.spec.ts` unmodified and green (18 tests). Suites green on the merged base, which now carries the specs of C.2, C.3 and C.4: `npm test` 2191 tests, `test:db` 219, `test:e2e` 394, and the three `DB_PASSWORD`-gated specs 7. No fixture edited. `previousType` derived from the `1770000000000` DDL (`varchar` with no length → `character varying`) — NOT from `omnifood`, see hazard 8. |
| **C.6** | evidence only, this PR | Independent verification **from the catalog, not from the units' specs**. **Zero** non-uuid `tenant_id` columns in the migration-built schema — 66 uuid, 0 non-uuid — measured by direct `information_schema` query rather than by the harness. **102 of 102** policies that reference `app.tenant_id` are in the setting-cast form, and **zero** carry a column-side cast. **Correction, and it matters:** the pattern first used for that second half was `%tenant_id::text%`, which is **vacuous** — `pg_policies` deparses the column side as `(tenant_id)::text`, with parentheses, so the pattern matches nothing and reports a clean zero whether or not the old form is present. Re-run with the correct discriminator `%(tenant_id)::text%` it still returns **0**, so the conclusion holds, but the original check could not have detected the defect and should not have been quoted as evidence. Note also that `%::text%` on its own is a false positive: all 102 policies contain `::text` legitimately, from `current_setting('app.tenant_id'::text, true)` inside the target predicate. `EXPLAIN` taken as `omnifood_schema_build_migrator` (non-superuser, `rolbypassrls = false`) on a sample including composite-key tables: `catalog_values` → Bitmap Index Scan on `idx_catalog_values_tenant_type_active`, `tenant_capability_event` → `uq_tenant_capability_event_revision`, `invoice_items` → `idx_invoice_items_tenant_invoice`, `device_sync_credentials` → `idx_device_sync_credentials_tenant`, all with `Index Cond: (tenant_id = (current_setting('app.tenant_id'::text, true))::uuid)`. **Hazard 5 demonstrated live, not restated**: the same `EXPLAIN SELECT * FROM catalog_values` returns a bare `Seq Scan` as `postgres` (bypassrls) — the tenant qual is absent, so the plan looks clean for the wrong reason. Two tables in the sample (`audit_integrity_alerts`, `customer_point_transactions`) correctly show **no qual at all** because they have `relrowsecurity = false` and zero policies; sampling an RLS-free table and expecting a qual is the false-negative trap in this check. |
