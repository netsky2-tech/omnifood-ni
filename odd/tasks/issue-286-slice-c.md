...
- `apps/admin_backend/src/migrations/1809100000000-RebindHumanAuthorizationTenantColumns.ts` — the closest template (slice B2's first slice unit), and now also `1809120000000-RebindLifecycleTenantColumns.ts` (C.2) and `1809130000000-RebindDeviceAndAlertTenantColumns.ts` (C.3), which are the closest templates for the units that follow. Note the deliberate timestamp gaps: the units are not stacked, so each claims a distinct timestamp to avoid a collision at merge.
