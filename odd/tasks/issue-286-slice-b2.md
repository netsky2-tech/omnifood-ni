...
| B2.3 | ~300–350 | **446** (415 new + 31 tracked across 8 files) — over budget, disclosed |
...
| B2.2 | `6f92c07` | `1809100000000` rebinds `human_auth_policy_epochs`, `human_auth_terminal_ack_history`, `human_auth_terminal_ack_floor`, `human_auth_recovery_tokens` and `human_auth_recovery_events` — 5 tables, 12 policy rows authored from the catalog — plus their five entity declarations. Ratchet **23 → 18**. Harness exit 0 in both scenarios with exactly the predicted movement: `non-uuid tenant columns 18`, `manifest 18`, `unlisted 0`, `stale 0`, `uuid col text casts 0`, `uuid tenant_id tables 47`, `entity uuid mismatches 0`, `RLS policies 102`. `human_auth_terminal_ack_floor` keeps its `@PrimaryColumn` while losing `length`, because `(tenant_id, terminal_id)` is that table's primary key. |
| B2.3 | _(this commit)_ | `1809110000000` rebinds the last four OHAC tables — `human_auth_verification_events`, `human_auth_rollout_cohorts`, `human_auth_tenant_publication_state`, `human_auth_policy_snapshots` — with 10 policy rows authored from the catalog, plus their four entities. Ratchet **18 → 14**, which ends *this slice*: what remains is slice C's tables. Harness exit 0 in both scenarios with exactly the predicted movement: `non-uuid tenant columns 14`, `manifest 14`, `unlisted 0`, `stale 0`, `uuid col text casts 0`, `uuid tenant_id tables 51`, `entity uuid mismatches 0`, `RLS policies 102`. Migration spec 5/5, conformance spec 18/18, previous unit's spec 5/5, `test:db` 36/200 including `ohac-publication-rls.db.spec.ts`, `test:e2e` 49/392, `npm test` 228 suites / 2096 tests. |
...
**The conformance spec pinned the old shape in THREE places, not the two this document enumerated.** Beyond the four `tenant_id` rows and the two entries in the width list, `starts the publication marker dirty at revision one` asserted `toMatchObject({ primary: true, length: 128 })` on the publication-state primary column. The writer found it, changed it to `{ primary: true, type: 'uuid' }` — swapping an obsolete width for the type, which asserts no less than before — and flagged it rather than treating the enumeration as closed. The lesson is the shape of the enumeration, not the count: **grep for the old shape across the whole spec, do not enumerate the places by reading one test at a time.**
