...
**Found on the way and filed instead of absorbed:**
- **#601** — `NegativeStockRegularizationService` orphan lifecycle (A4 / F8). Kept out of this PR deliberately: it is a `PROVISIONAL → REGULARIZED` retrocalculation with a lineage hash, and it is *masked* by #519 — it only becomes observable once sales produce movements again.
- **#602** — a fresh install gets the `authority_%` tables **without** their immutability triggers. `migration49_50` installs them only on the upgrade path; the generated entity DDL does not (`grep -c authority_recipe_versions_block_update lib/data/database/app_database.g.dart` → 0) and the registered `onCreate` callback (`inventoryMovementAppendOnlyCallback`) covers movement tables only. So upgraded terminals enforce append-only authority facts and newly enrolled ones do not — found while building U6's migrated-DB fixture, and the reason U6 does not depend on those triggers. Fix = converge both paths + write the install-parity assertion `ohac_delivery_install_parity_test.dart:20-35` already proves for its own tables.
