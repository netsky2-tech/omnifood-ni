...
| **1** | Inventory & Kardex — **DONE, #345 + #346 + #347** | 10 | 27 | `inventory_kardex` (8), `inventory_purchase_documents` (4), `inventory_sync_outbox` (3), `inventory_sync_receipts` (4), `inventory_remediation_receipts` (2), `kardex_correction` (1), `kardex_recalculate_queue` (1), `production_batch_history` (4), `product_import_sessions` (0), `staging_importacion_productos` (0) |
| **2** | Onboarding & fiscal — **DONE, #360 + #369 + #363 + #364** | 10 | 21 | `onboarding_activation_attempts` (4), `onboarding_activation_check_results` (4), `onboarding_activation_follow_ups` (4), `onboarding_telemetry_events` (4), `onboarding_sessions` (0), `onboarding_idempotency_records` (0), `onboarding_template_applications` (0), `onboarding_template_seed_links` (0), `fiscal_config_revisions` (4), `sys_parametros_config` (1) |
| **3** | OHAC / human authorization — **DONE, #368 + #372 + #374** | 9 | 22 | `human_auth_policy_epochs` (2), `human_auth_policy_snapshots` (2), `human_auth_recovery_events` (2), `human_auth_recovery_tokens` (3), `human_auth_rollout_cohorts` (3), `human_auth_tenant_publication_state` (3), `human_auth_terminal_ack_floor` (3), `human_auth_terminal_ack_history` (2), `human_auth_verification_events` (2) |
...
**The estimate in the table above is wrong, and Slice A falsified it.** This section projected ~3 pull requests for the whole of Phase 2. The first slice alone took **three**, totalling **1,701 review-facing lines** (684 + 353 + 664). What dominates is not the per-policy SQL — which the shared emitter does collapse into data — but the **specs** (402 of the emitter's 684, 365 of the slice's 664) and the earlier-migration fixes. Expect **~15–20 pull requests** for Phase 2, not 3. Both of the first two exceed the 400-line budget for the TDD-pairing reason: a spec cannot be reviewed apart from the implementation it pins.

**A rule every remaining slice must follow.** Each slice must also make the earlier policy-creating migrations for **its own tables** type-aware, through the shared resolver, with **one resolution per table** and never one shared across tables. Without it, scenario 2 of the schema build check fails: on a partial ledger the earlier migration re-runs and emits the predicate matching the column type as it *was*, against a column that is now uuid. Slice A needed **six** such migrations, not the one this plan assumed, and they were found one failure at a time by the check. That work belongs inside the slice, not in a separate unit, because it exists only to serve that slice's tables.

**Slice A's shape, for the remaining slices to copy.** Three chained pull requests, each green on its own, in this order — the order is load-bearing:
1. the shared capability (#345, the emitter; inert, no consumer);
2. the earlier migrations made type-aware (#346; provably behaviour-neutral, because every column is still varchar so the resolver returns the text form);
3. the slice itself (#347; now scenario 2 passes, because it depends on 2).
Reversed, the middle link would be red, and a red link in a chain is a queue rather than a review unit.

**Slice B1 is merged (2026-09-19)**, as **four** chained pull requests plus one recovery pull request: `#360` (the capability), `#369` (the two earlier migrations made type-aware — this is `#361` re-applied, see the hazard below), `#363` (five onboarding/fiscal tables, 20 policies, 9 entities, 2 fixtures) and `#364` (`sys_parametros_config` behind its view). The ratchet went **34 → 23**. Slice B1's own plan of record, with per-unit evidence, is `odd/tasks/issue-286-slice-b1.md`.

**B1 added a fifth harness layer, and it immediately found a defect in already-merged work.** The schema build check compared entity columns for **existence**, never for **type**, so a column could reach `uuid` while its mapping entity still declared the old shape and every check stayed green. The new assertion is deliberately **one-directional** — for every base table whose `tenant_id` is uuid, the mapping entity must declare uuid — because the census found 14 disagreements in two opposite directions (50 agreed): eight where the column had arrived and the entity was left behind, and six where the entity was already at the destination via its `@ManyToOne(() => Tenant)` relation while the column was not. The second group is the in-transit state the ratchet already tracks; gating on it would flag transit as a defect. **Four of the eight were slice A's own leftovers** (`inventory_remediation_receipts`, `inventory_sync_outbox`, `inventory_sync_receipts`, `production_batch_history`), and the entity side must be read from TypeORM metadata, never from entity source text: thirty entity files declare `tenant_id` twice and the relation-derived type wins.

**Slice B1's slice had to be split, which is the estimate correction this slice earned.** 21 policies did not fit one review unit: the first half alone cost **664 lines** for 20 policies, over the 400 budget, so the sixteen units of this plan should be read as *at least* one pull request each, and the whole of Phase 2 as materially more than the 15–20 recorded after slice A.

**Two assumptions worth carrying forward, both falsified by measurement rather than argument.**

1. **A fixture census done by grepping class names is structurally blind.** B1 was delegated with the claim “no fixture needs changing”, based on `grep 'synchronize: true'` plus the entity class names. Two e2e fixtures failed precisely because they nest the entities **behind a helper function**, so the names never appeared in the file the grep read. The checkable class is **“a fixture that synchronize-builds from an entity whose `tenant_id` type changes”**, and indirection through an import is enough to hide it.
2. **“Change only the type on the entity declaration” is impossible.** TypeORM validates that a `uuid` column does not carry `length`, so the first harness run after the conversion failed with `Column tenantId of Entity ActivationAttempt does not support length property`. Every converted declaration drops its length along with the type.

**Slice B2 is merged (2026-09-19)**, as **three** pull requests, all based directly on `main`: `#368` (the five earlier OHAC migrations made type-aware), `#372` (five tables / 12 policies) and `#374` (the last four tables / 10 policies). The ratchet went **23 → 14**. Slice B2's own plan of record is `odd/tasks/issue-286-slice-b2.md`.

**B2 changed the chain shape on purpose.** B1's links were stacked, and that is what made the silent retarget failure land a commit on a parent branch. Every B2 link is green on its own against `main`, so they were landed **sequentially off `main` with no parent**, which removes that whole class of hazard. Two links were enough instead of three because the shared capability the slices need was merged with B1: `#360`'s emitter view support and the entity/schema assertion were already in place. **Remaining slices should prefer this shape** unless a link genuinely is not green alone.

**B2 added no new harness layer, and that is the point.** The detector B1 added — every base table whose `tenant_id` is uuid must be mapped by an entity declaring uuid — did the work here: it is what made the four entity conversions in each slice unit mandatory rather than optional, and it is what fails the build when a slice converts a column and forgets the entity.

**A conformance spec is the strongest available check for an entity conversion, and it pins the old shape in more places than a careful reader enumerates.** `human-auth-entities.spec.ts` asserts each entity's declared columns against what the migrations declare. It broke in **three** places, not the two the slice document listed: the per-table `tenant_id` rows, the `varchar` width list, and a `toMatchObject({ primary: true, length: 128 })` on a primary column. The lesson recorded: **grep the whole spec for the old shape; do not enumerate its occurrences by reading one test at a time.**

**Independent verification for B2 came out of the catalogue, not out of the specs.** With every `human_auth_*` column rebound: 22 OHAC policies, **0** without the setting-side uuid cast; and `EXPLAIN` under `enable_seqscan = off` as the **non-bypassing** migration role shows `Index Cond` on all three probed tables — including both composite-primary-key tables, whose primary indexes PostgreSQL rebuilt on the type change. A superuser would show the same query with no qual at all, because RLS predicates are not added for a bypassing role, which is why the role matters. `gh pr edit <n> --base main` **can fail silently** — it printed a GraphQL deprecation notice from the unrelated Projects API and exited 0 without changing the base — and chaining `&&` after it squash-merged a child into its **parent branch** instead of `main`. A merged pull request cannot be retargeted afterwards (`422 Cannot change the base branch of a closed pull request`), so recovery is a cherry-pick onto `main` plus a fresh pull request. Retarget through the REST API, **read the base back**, and retarget **before** pushing, because a retarget does not itself trigger `pull_request` workflows and a push made first leaves the new head with no CI run. Because a squash merge breaks the child's merge base, `main` must be merged **into** the child first.

**Merge-sequence hazards, recorded because they cost a pull request.** `gh pr edit <n> --base main` **can fail silently** — it printed a GraphQL deprecation notice from the unrelated Projects API and exited 0 without changing the base — and chaining `&&` after it squash-merged a child into its **parent branch** instead of `main`. A merged pull request cannot be retargeted afterwards (`422 Cannot change the base branch of a closed pull request`), so recovery is a cherry-pick onto `main` plus a fresh pull request. Retarget through the REST API, **read the base back**, and retarget **before** pushing, because a retarget does not itself trigger `pull_request` workflows and a push made first leaves the new head with no CI run. Because a squash merge breaks the child's merge base, `main` must be merged **into** the child first. **B2 then landed without any of this**: every link green on its own against `main`, no parent, no retarget.

**One more process hazard: `npm run lint` in `apps/admin_backend` is `eslint "{src,apps,libs,test}/**/*.ts" --fix`.** Running it as a verification step rewrote 48 tracked files unrelated to the slice and exited 0 while doing it. Verify lint with a direct `npx eslint <paths>` call.
