...
- El lookup oficial BCN por `invoiceDate` ya existe en backend + POS como flujo opt-in con fallback manual offline-safe, y el backend soporta transporte proxy/configurable para consultar BCN cuando falta una tasa exacta local. La caché BCN es la tasa persistida por `effective_date` exacta; si BCN no publica la fecha exacta solicitada, el backend devuelve `NotFound` para ruta manual, sin usar último día hábil ni fechas cercanas.
...
- Servicio `BcnFxService.obtenerCambio(fecha)`: API BCN + caché local persistida por fecha exacta (`effective_date`). El tipo de cambio se toma por `fecha_emision`; **la BCN no cambia el cambio por feriados** (D2). **No** asumir fallback a "último cambio hábil anterior" salvo que una fuente oficial lo respalde. Si la fuente mensual carece de cambio para la fecha exacta, el backend devuelve `NotFound` para captura manual/offline-safe y no persiste una tasa de otra fecha.
