...
- [ ] **Fuente/caché BCN determinista**: servicio que obtiene el tipo de cambio oficial BCN por `fecha_emision`, con caché local. Si la fuente carece de cambio para una fecha, la regla debe estar **respaldada por fuente oficial** (no inventar el cambio, no asumir fallback a "último cambio hábil" — ver D2).
