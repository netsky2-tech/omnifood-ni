...
- [x] **Explosión BOM con cantidad bruta vs neta y factor de rendimiento validados.** *(Slice 2.2 — el `MovementEngine` ahora consume el `grossQuantity` del documento versionado escalado por `saleQuantity / yieldQuantity`, valida `yieldQuantity > 0`, `grossQuantity > 0`, `technicalShrinkPct` en [0,100) y `netQuantity == gross*(1-shrink/100)` a 4dp antes de generar movimientos, y valida compatibilidad de UOM del componente contra la unidad base de consumo del insumo.)*
...
- **Ingesta de versiones de receta en el backend**: el POS sincroniza hacia `/inventory/recipes/versions` (ahora incluyendo `componentUom` en el payload), pero el backend aún no expone un controlador que la consuma. La validación/conversión de UOM del lado backend queda diferida a ese endpoint.
