...
| 3a — Kardex Inmutable | **Parcial** | Slice 3a.1 enforces backend `inventory_kardex` append-only immutability, and Slice 3a.2 now validates historical balance baselines at migration time plus same-stream running-balance inserts with advisory-lock serialization. Remaining work: local SQLite parity and costing freeze. |
...
| §2.3 Kardex Inmutable | Append-only, auditoría, sin DELETE/UPDATE | 3a | Parcial (backend `inventory_kardex` now rejects UPDATE/DELETE at DB level and enforces validated running-balance continuity for same-stream inserts; local parity/cost freeze still pending) |
...
| NFR Concurrencia | SERIALIZABLE / FIFO por ítem | 3a, 3c | Parcial (3a.2 serializes backend inserts per `tenant_id + insumo_id` to protect the running-balance invariant; broader deterministic sync/FIFO remains in 3c) |
...
| 3a | [Kardex Inmutable](batch_03a_kardex_invariants.md) | Append-only, esquema auditoría, invariantes, concurrencia | Split de Batch 3 | Parcial (backend append-only + running-balance invariant slices done; local parity/cost freeze pending) |
